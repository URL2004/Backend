...
기준 커밋은 `e13cbf48f30b39f455be0cfc0ed03394158136a5`다. 로컬 카피킬러 테스트 API와 관련 라우트·마운트·로컬 파일은 이 출시 범위에 포함하지 않는다.
...
1. `fix/humanize-engine-v2`를 `deploy/staging-backend`에 반영한다.
2. Render `ai-backend-staging`에 `LLM_ACTIVE_PROVIDER=gpt`, `HUMANIZE_ENGINE_V2_ENABLED=1`, `OPENAI_API_KEY`, 32바이트 이상의 `OPENAI_SAFETY_SALT`를 설정한다.
...
스테이징의 자동·수동·카피킬러 기준을 모두 통과한 경우에만 `release/prod-maintenance-test`에 반영한다. 배포 직전 운영 `/healthz`의 `activeJobs`가 0인지 다시 확인한다. 100% 트래픽에 적용하고, 기준 초과 시 `HUMANIZE_ENGINE_V2_ENABLED=0`으로 즉시 복귀한다.
