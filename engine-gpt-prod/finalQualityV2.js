...
async function runSemanticDocumentAuditInternal({
  ...
  return bindSemanticValidation({
    ...
    repairStyleWarnings: [...new Set(reports.flatMap(report => report.repairStyleWarnings || []))],
    unchangedRepairCount: reports.reduce((sum, report) => sum + (report.unchangedRepairCount || 0), 0),
...
async function retryGeneralSurface({
  ...
  const system = [
    ...
    '문제가 있는 대상은 동의어 교체에 머물지 말고 충분히 고친다. 변경량보다 자연스러움·호응이 우선이다.',
    `${strengthLabel} 모드의 변화량은 서버가 결과에서 계산한다. 명료한 동사를 “~하는 일을 이어가다·시작하다”로 늘이거나 수식어를 끼워 수치를 채우지 않는다.`,
    noEffectRecovery
      ? '앞선 회복도 효과가 부족했다. 지정 문장 중 아직 기계적 연결·중복·과밀이 남은 곳을 넓게 검토한다. 이미 자연스러운 문장까지 바꾸거나 개수를 채우기 위해 늘이지 않는다.'
      : '',
    `수정 대상 문장 번호=${targetText}. 번호는 SOURCE와 CURRENT의 일반 문장 순서 기준이다.`,
    paragraphLow
      ? `아직 검토할 일반 산문 문단=${untouchedParagraphs.join(',') || '서버 표시 문단'}. 이 문단의 실제 문제도 놓치지 말고 개선하되 분포 수치만을 위해 정상 문장을 바꾸지 않는다.`
      : '',
    plan.requestStrength === 'advanced'
      ? '고급은 여러 문단의 지정 대상을 빠짐없이 검토한다. 각 문단의 실제 문제는 충분히 재작성하되 역할·순서를 유지하며, 명료성이 나빠지는 어순 변경은 하지 않는다.'
      : '',
    ...
    structuralLow
      ? '문장 구조 변화 수치는 참고다. 대상에 기계적 연결·과밀·중복이 남아 있을 때만 충분히 재구성한다. 이미 자연스러운 문장의 도치·명사화로 수치를 채우지 않는다.'
      : '',
    remediationLow
      ? [
...
async function retryConservativeSentenceSurface({
  ...
  const system = [
    ...
    '기계적 연결·중복·과밀이 남은 문장은 같은 뜻 안에서 충분히 재구성한다. 이미 자연스러우면 유지한다. 명사화·도치로 변화량을 채우지 않고 주어·수식어·조건과 서술어의 호응을 우선한다.',
...
function refinementIssueInstruction(item) {
  ...
  if (item?.code === 'introduced_mixed_script_word') return '한글 단어 중 새로 한자로 바뀐 글자는 SOURCE에서 유일하게 대응되는 어절과 문맥이 확인될 때만 원문 표기를 복원한다. 원래의 일본어·중국어·한자 표기, 고유명사, 인용과 코드는 바꾸지 않는다.';
