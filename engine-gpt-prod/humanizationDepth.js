...
function normalizePerceivedSubstantive(value) {
  return stripLockTokens(value)
    .normalize('NFC')
    .toLowerCase()
    // 문장 첫 기능성 접속어는 사람이 체감하는 실질 재작성량이 아니다.
    // 이를 별도 토큰으로 세면 접속어 주입이 깊이 점수를 올리고, 자연성
    // 수리가 제거할 때 오히려 후보가 거절되는 역보상이 생긴다.
    .replace(/(^|[.!?。！？]\s*)(?:(?:그리고|또한|아울러|더불어|따라서|그러므로|이에\s*따라|한편|나아가|결론적으로|이러한|이를\s*통해)\s*[,，]?\s*)+/gu, '$1')
    // `그다음,`, `검토한 뒤,`처럼 문장 첫머리에 새로 붙인 순차 표지도
    // 실질 재작성 공로로 세지 않는다. 쉼표 없는 `26주 이후에는` 같은
    // 실제 시간 관계는 의미 정보이므로 이 중립화 대상에서 제외한다.
    .replace(/(^|[.!?。！？]\s*)(?:(?:그\s*다음|다음으로|그\s*뒤|그\s*후|이후|[가-힣A-Za-z0-9·/-]{1,24}(?:한|된|친|운|은)\s*(?:뒤|후))(?:\s*[,，]\s*|\s+))+/gu, '$1')
    .replace(/통하여/gu, '통해')
    .replace(/대하여/gu, '대해')
    .replace(/위하여/gu, '위해')
