...
function meaningPreservationLines() {
  return [
    ...
    '가능성과 화자의 의견은 수식 범위와 함께 보존한다. “생각이 든다”를 근거 없는 단정으로, “제공할 수 있고”를 실제 제공 사실로 바꾸지 않는다. “조금 어렵다”의 정도를 생략해 “어렵다”로 강화하지 않는다. 주체가 다른 뒤 절의 “수 있다”만으로 앞 절의 가능성을 대신하지 않는다. 같은 주체의 “자료를 검색하고 복사할 수 있다”처럼 뒤의 가능성이 두 행동을 함께 수식하는 결합은 허용한다. 같은 의미의 자연스러운 표현 교체는 허용한다.',
    '중복을 줄일 때 앞 문장이 뒤 문장에 제공하는 시점·수단·대상·조건을 지우지 않는다. 다른 문단에 같은 정보가 있어도 현재 설명의 연결 정보가 불필요해지는 것은 아니다. 문장 결합이 허용된 모드에서는 그 정보를 뒤 문장에 담아 간결하게 합칠 수 있고, 결합이 금지된 모드에서는 연결 문장을 유지한다. 반복 문구 자체를 모두 고정하거나 떨어진 정보를 추측해 삽입하지 않는다.',
