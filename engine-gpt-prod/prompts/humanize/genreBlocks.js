...
function academicReportBlock(register) {
  return [
    ...
    'LaTeX·유니코드 수식, 행렬, 변수, 기본행 연산, 코드·의사코드는 문장 재작성 대상이 아니다. 원문에 있는 표현은 그대로 두고, 원문부터 비어 있는 식이나 구현 내용을 새로 만들어 보충하지 않는다.',
    ...
    '문장을 나눈 뒤 “연구 결과·이 결과·~을 시사한다”의 연구 주체나 지시 대상이 사라지지 않게 한다. 본 연구와 인용 연구를 서로 바꾸지 않고, 필요하면 앞 문장과 다시 연결한다.',
    '“분석하였다”를 “분석 대상으로 삼았다”로 바꾸어 수행 단계를 약화하지 않는다. 시간·맥락의 “이때·뒤·후”를 인과 결론의 “따라서·그 결과·~자”로 바꾸지 않고, 주체 뒤의 “이후”를 문장 앞으로 옮겨 시간 기준을 흐리지 않는다.',
    '“환경을 일부 공유한다”를 “환경의 일부를 공유한다”로 바꾸는 식으로 범위 조사를 흐리지 않고, 연구가 어떤 대상에 한정된다는 문장을 그 대상이 연구에 머문다고 뒤집지 않는다.',
    '개념을 일상어로 낮출 때도 “재다·메우다”처럼 논문에 어색한 구어를 쓰지 않고, 플랫폼·도구가 사람에게 무엇을 “~해 준다”고 의인화하지 않는다.',
    '“성장세를 나타내기 시작했다”처럼 의미를 겹친 관료적 명사화, “결제를 끝내다” 같은 가벼운 완료 동사, “시설을 구축하다”를 “마련하다”로 낮추는 부정확한 치환을 새로 만들지 않는다.',
    `원문 종결체=${register}.`
  ].join('\n');
}
...
function essayApplicationBlock(register, profile, requestStrength = '') {
  ...
  if (profile === 'resume_application') {
    lines.splice(3, 0,
      ...
      '한 업무 묶음을 문단 경계에서 끊지 않는다. 회로 작성과 그 레지스터·출력 검증처럼 핵심어가 이어지는 문장들은 같은 역할 문단에 두고, 다음 직무 단계가 시작될 때만 빈 줄을 둔다.',
      '재구성한 티를 내려고 “완료된 뒤·검토한 후·확인한 다음”을 연속해서 새로 넣지 않는다. SOURCE의 실제 순서는 유지하되 접속 부사 없이도 이어지는 문장은 직접 연결한다.',
      '“우선순위를 먼저 판단”, “자료 수정을 맡을 역할을 나누다”처럼 순서 의미나 역할 서술을 겹치지 않는다. “손발이 빠르다” 같은 정상 연어를 임의로 분해하지 않는다.',
      '주어가 생략된 “믿고 맡길 수 있는 선생님” 앞에 아이들을 새 주어로 붙여, 아이들이 선생님에게 무엇을 맡기는지 모호한 문장을 만들지 않는다.',
