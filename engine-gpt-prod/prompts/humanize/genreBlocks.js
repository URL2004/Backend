...
function polishBlock(register, documentProfile) {
  return [
    ...
    '정상 문장을 비슷한 길이로 맞추기 위해 합치거나 나누지 않는다. 구두점 누락을 고칠 때도 의미 단위에 따라 길이 차이를 남긴다.',
...
function studentRecordBlock(register) {
  return [
    ...
    '제목·항목처럼 따로 놓인 행과 기존 줄바꿈을 합치거나 새 문단으로 재배치하지 않는다.',
    '원문의 짧은 관찰문을 앞뒤 문장과 합치거나 긴 문장을 중간 길이 문장들로 균등 분할하지 않는다.',
