...
function academicReportBlock(register) {
  return [
    ...
    '학술 본문의 구어적 별칭·게임식 표현·과장 은유는 직접 인용이나 정식 구성개념이 아니면서, 같은 문장 또는 제공된 앞뒤 문맥에 실제 행위·상태가 명시된 경우에만 그 의미와 정도를 유지한 중립 표현으로 바꾼다. 대응 관계가 불확실하면 전문 용어를 추정하지 말고 원래 표현을 유지한다.',
    '“뜬금없는 결합”이나 “폭발적인 반응”처럼 장르에 명백히 어긋난 표현도 불일치·반응의 강도를 낮추지 않는 범위에서만 고친다. 이론 용어를 추정해 다른 개념으로 바꾸지 않는다. 외부 지식으로 숨은 절차나 이론 용어를 만들어 풀어 쓰지 않는다.',
    '연구 목적 한 문장에 목적·핵심 단서·작동 과정·검증 조건이 겹쳐 실제로 읽기 어려우면 논리 역할에 따라 한두 문장으로 조정한다. 명사형 관찰·기록 형식이거나 나누면 변수·조건의 대응이 흐려질 때는 문장 수를 억지로 늘리지 않는다.',
