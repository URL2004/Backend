...
const { academicStyleLines } = require('../academicStyle');
...
function academicReportBlock(register) {
  return [
    '[원문 장르: 학술·보고서·설명문]',
    academicStyleLines('report_assignment').join(' '),
    '논지·내부 참조·논리 연산자를 보존한다. “~자체보다”→“~에서 나아가”, “~에 그치지 않고”→“~이/가 아니라”로 바꾸지 않는다.',
    '추정·전망과 주체·대상을 유지한다. 연구자의 표집을 시료의 행동으로, 설명을 명령으로 바꾸지 않는다. 논증 단계 안에서 편집하고 표·그림 제목·캡션·셀의 압축도를 보존한다.',
    '최적화·상관관계·원인 분석·재현성 검증·수치화·데이터 해석을 확인·관계·조정·재다·메우다로 약화하지 않는다. 도구도 의인화하지 않는다.',
    '외부 지식으로 사실·숨은 절차·이론 용어를 추정 교정·보충하지 않는다. 구어적 별칭·은유는 대응 행위가 명확할 때만 중립화한다.',
    '과밀한 목적문은 목적·단서·과정·검증 조건·범위·활용의 대응을 지키며 나눈다. 주제어 반복은 중복 주장이 아니다. 본 연구·인용 연구·결과의 지시도 유지한다.',
    '“분석하였다”→“분석 대상으로 삼았다”, “뒤·후”→“그 결과·~자”, “시설을 구축하다”→“마련하다”처럼 행위·인과·강도를 바꾸거나 범위 조사를 뒤집지 않는다.',
    `원문 종결체=${register}.`
  ].join('\n');
}
