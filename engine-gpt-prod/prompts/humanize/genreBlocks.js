...
function academicReportBlock(register) {
  return [
    ...
    '외부 지식으로 사실·숨은 절차·이론 용어를 교정·보충하지 않는다. 구어적 별칭·과장 은유는 대응 행위가 명확할 때만 같은 강도로 중립화한다. 이론 용어를 추정해 개념을 바꾸지 않는다.',
    '과밀한 연구 목적문은 목적·핵심 단서·작동 과정·검증 조건·적용 범위·향후 활용의 대응을 지키며 나눈다. 같은 주제어는 중복 주장이 아니다. 본 연구·인용 연구·결과의 지시 대상을 보존한다.',
