...
function verifySemanticValidation(report, { source, candidate, requireDigest = false } = {}) {
  ...
  if (validation?.version === VERSION) {
    if ((source != null && validation.sourceDigest !== textDigest(source))
        || (candidate != null && validation.candidateDigest !== textDigest(candidate))) {
      return { status: 'stale', rank: 1 };
    }
  ...
  if (report.ran === true && report.pass === true) return { status: 'pass', rank: 2 };
...
module.exports = { VERSION, textDigest, bindSemanticValidation, verifySemanticValidation, projectSemanticValidation, finalValidationWarnings };
