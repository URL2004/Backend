...
function buildFactMap(rawText) {
  ...
  // ★ factast.tokOf로 위임(무제한 — i≥676 깨짐 '{' 제거). i<676은 ⟦Faa⟧~⟦Fzz⟧로 기존과 동일. 기본 on(FACT_AST=0이면 기존).
  const tokOf = process.env.FACT_AST !== '0'
...
// ★ factast.placeholdersIn(/⟦F[a-z]{2,}⟧/ — 3글자 확장 토큰도 검출). tokOf와 반드시 함께 전환. 기본 on(FACT_AST=0이면 기존).
function placeholdersIn(text) {
  if (process.env.FACT_AST !== '0') return require('./factast').placeholdersIn(text);
