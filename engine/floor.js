...
const META_NOTE_RE = /(메모\s*:|(?:본문만?|출력|원문대로|지시대로|메모|재작성[한된])[^.”"]{0,10}밝힙니다|지시에\s*따라|삽입하지\s*않|본문만|머리말\s*(?:없이|금지)|설명\s*없이\s*(?:출력|작성)|위\s*(?:지침|규칙|룰|프롬프트)|요청하신|재작성한?\s*(?:칼럼|글|문단|텍스트)\s*(?:만|을|를|입니다|:)|원장[은는이가]\s|승인\s*근거|added_claim|distortion|claim\s*ledger|\bREWRITE\b|수정\s*문장\s*[:：]|#{1,6}\s*판정|#{1,6}\s*근거)/i;
