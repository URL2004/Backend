...
const TEMPLATE_FAMILIES = [
  ...
  // ★ 2026-06-16 품질리포트 실측 추가: 격식·자소서에 끼던 극적 단정 punch(#2·#16).
  { key: '단순하지않다-punch', re: /(현실|문제|상황|세상|답|구조|이야기|해결책|세계)[은는이가]?\s*(결코\s*)?단순하지\s*않(다|았다)/g, budget: 0, desc: '「현실은 단순하지 않다」류 단정 punch' },
  { key: '뒤흔들렸다-punch', re: /(정책|판|구조|질서|세계|모든\s*것|관계|일상|판도)[이가]?\s*(뒤흔들렸|흔들렸|무너졌|뒤집혔|송두리째\s*바뀌)/g, budget: 1, desc: '「정책이 뒤흔들렸다」류 극적 단정' },
