...
· "필자가 보기에"(3인칭 상투구), "과연 ~인가?", 교훈 요약 마무리("~가 중요하다/필요하다"로 문단 닫기 반복) — 단 사용자 메모의 입장을 "나는/내가 ~라고 본다"로 드러내는 1인칭 판단은 금지가 아니라 권장이다(styleBlock 따름). 여기서 막는 건 "필자가"식 3인칭 상투구와 같은 표현의 반복이다.
...
· ★★비인칭 분석 나열 금지(분석·시사 글의 간접화법 직격): "기업은/데이터는/조직은/시장은 ~된다/~이다"로 끝나는 3인칭 객관 서술을 죽 이어붙이지 마라 — 같은 비인칭 단정의 행렬이 카피킬러 '간접화법·비인칭' 태그의 정체다. 필자의 판단을 능동으로 드러내라. ★사용자 메모에 입장이 있으면 그걸 글 전체를 관통하는 한 줄 시각으로 끌고 가서, 객관 서술 사이사이 "나는 ~라고 본다", "내가 보기엔 ~", "여기서 판단이 갈린다", "정작 따져야 할 건 ~다"처럼 1인칭 판단 주체를 드러내며 비인칭 수동("~된다/~로 평가된다/~게 된다")을 능동 단정으로 바꿔라. ★단 같은 입장 표지를 반복하면 그것도 템플릿이 되니 글마다 다르게 변주하라(한 표현 2회 이하). 원문·메모에 없는 새 견해·사실은 만들지 마라.
...
· 분량: 문단 ${Math.max(1, Math.round(slot.targetChars / 330))}개, 공백 제외 약 ${slot.targetChars}자(재료가 충분할 때의 목표). 단 재료(주장·근거·원문 문단)가 빈약하면 짧아도 된다 — 분량을 채우려고 새 사실·부연을 지어내면 실패다(짧음 < 날조).
...
async function genreTransferV2(rawText, { skeleton = 'debate_explainer', evidence = '', userNotes = '', lang = 'ko', lengthMode = 'keep', signal } = {}) {
  ...
  pairing = checkEvidencePairing(doc, evidenceList);    // 짝 교정은 scrub '전'에(아래 final 재측정과 별개)
  ...
  pairing = checkEvidencePairing(doc, evidenceList);   // scrub 후 최종 짝 상태(반환·차단 판단 기준)
