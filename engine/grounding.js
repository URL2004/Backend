...
function buildGroundingPrompt(segText, anchors, lang = 'ko', mode = 'assignment') {
  ...
  // ★ 격식 FLOOR 완화 실측 폐기(2026-06): 평가/수사의문문 주입은 카피킬러를 못 내림(EV 73→83% 오히려 악화).
  //   "필자가 보기에"·"과연~인가?"를 넣은 바로 그 문단조차 카피킬러는 여전히 "주관성 배제"로 잡음 → 키워드가 아닌 글 전체
  //   객관 레지스터를 통계적으로 봄(흩뿌린 1인칭이 전체 문어체 통계를 못 바꿈). "과연~인가?"는 AI 전형 패턴이라 역효과.
  //   결론: 비인칭 판단문 grounding(73%)이 격식 모드 최선. 사장님 불가침 원칙(평가·견해 추가 금지) 복원.
...
[핵심 작업 — 이것을 최우선으로]
· 막연한 서술("~이 중요하다", "~할 수 있다", "~가 필요하다", "~도 하나의 요소다")을 **판단문**으로 바꿔라 — 문단 안에 이미 있는 대조·조건·한계·우선순위를 더 또렷하게 드러내는 것이다.
· ★판단문의 구조를 문단마다 다르게 만들어라. 대조형, 한정형("~일 때만 성립한다"), 조건형("~가 빠지면 ~는 무너진다"), 우선순위형("~보다 ~가 먼저다"), 단정형("결국 ~가 좌우한다") 등 서로 다른 형태를 돌려 쓰되, ★"~가 아니라 ~", "핵심은 ~", "문제는 ~", "~을 가르는 것은" 같은 한 가지 구문을 반복하지 마라 — 같은 구문이 글에 여러 번 나오면 그 자체가 기계 신호다. 이 지시문의 표현을 그대로 베끼지 말고 문단의 실제 내용으로 빚어라.
· 판단의 재료는 반드시 **그 문단 안에 이미 있는 내용**이어야 한다. 새 내용을 넣지 마라.
