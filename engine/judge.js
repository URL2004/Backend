...
async function semanticJudge(rawText, outputText, ledger, { lang = 'ko', signal, allowedExtra = '', mode = '' } = {}) {
  ...
  // ★ 격식 모드(assignment/thesis) FLOOR 완화(사장님 승인): 학술 글에서 글쓴이가 기존 원장 내용에 붙이는
  //   해석·평가·관점·수사의문문("필자가 보기에/핵심은/주목할 만하다/과연 ~인가/오히려")은 "새 사실"이 아니라 논지의 명시화 →
  //   (3)의 '평가'를 위반에서 제외. 단 (1)새 사실·수치·고유명사·인과 날조와 (2)왜곡은 그대로 엄격 차단.
  const formal = (mode === 'assignment' || mode === 'thesis');
  const formalKo = formal ? ' ★[격식 학술 모드] 위 (3)에서 "평가"는 제외한다 — 글쓴이가 *기존 원장 내용*에 붙이는 해석·평가·관점·강조·수사의문문("필자가 보기에/핵심은/주목할 만하다/과연 ~인가/오히려 ~다")은 위반이 아니다(논지의 명시화일 뿐). 새 사실·사건·수치·고유명사·인과관계를 사실처럼 도입할 때만 위반으로 잡아라.' : '';
  const formalEn = formal ? ' ★[FORMAL ACADEMIC MODE] In (3), evaluation/interpretation/rhetorical questions the author adds ABOUT EXISTING ledger content ("in my view", "the key is", "is it really enough?") are NOT violations — they only make the existing argument explicit. Only flag introducing NEW facts/events/statistics/proper nouns/causal claims as fact.' : '';
  const system = lang === 'en'
