...
const GRAMMAR_PATTERNS = [
  ...
  },
  {
    ...
    // 정상적인 "다. 또한 다음 절..."을 hard error로 세지 않는다.
    // 접속어가 실제 문서 끝에 홀로 남은 경우만 미완 구조로 본다.
    re: /다\.\s*(?:있으며|그리고|또한|때문에|따라서|그러나)\s*$/g,
  ...
  },
  {
    ...
    hard: false,
    label: '접속어미 뒤 쉼표 파편',
    // 한국어에서 종속절 뒤 쉼표는 정상일 수 있다. 쉼표 뒤 완결 절이 없이
    // 문서/문단이 끝나는 경우만 파편 후보로 기록한다.
    re: /(?:며|으며|고|지만|는데|면서|하여|해서),(?=\s*(?:$|\n{2,}|[.!?。！？]))/g,
