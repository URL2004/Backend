...
function buildSystemPrompt(mode = 'assignment', lang = 'ko', { speakerType = 'individual', lengthPolicy, userNotes = '', register = 'mixed' } = {}) {
  ...
  return [
    ...
    '· ★비인칭·수동 → 능동·주체(카피킬러 "간접화법·비인칭" 직격): "~된다/~여겨진다/~이루어진다/~할 수 있다/~되곤 한다" 같은 비인칭·수동·정형 종결을 주체가 드러나는 능동으로 바꿔라. 예) "효율성에 큰 비중을 두었다"→"기업들은 효율성에 매달렸다", "성과가 나타난다"→"몇 년이 지나서야 성과가 얼굴을 내민다". 단 없는 주체·사실은 만들지 마라.',
