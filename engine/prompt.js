...
const TONE_KO = {
  ...
  blog: '친근한 구어체 블로그 말투. 문단은 3문장 이하로 짧게, 사이에 빈 줄. ★문장 길이를 들쭉날쭉하게(burstiness) — 짧은 문장과 긴 문장(50자+)을 섞고 같은 길이대 2연속 금지. 의태어(확/슬쩍/푹/툭)도 자연스럽게. ★★【문체 절대 규칙】모든 문장을 반드시 해요체로 끝내라 — 한다체(~다/~이다/~한다/~된다)·합니다체(~습니다)·명사로 딱 끊는 체언 종결("결국, 신뢰." 같은 식)은 한 문장도 쓰지 마라(카피킬러가 "간접화법·비인칭"·문체혼합으로 잡는다). 종결 다양화는 오직 해요체 안에서만: ~예요/~이에요/~거든요/~더라고요/~죠/~잖아요/~네요/~ㄹ게요/~ㄹ까요?를 골고루 돌려 쓰고, ★같은 종결어미 2연속 금지, 특히 "~거든요"·"~요"만 반복하지 마라. 가끔 의문문(…까요?/…잖아요?)으로 리듬을 바꿔라. ★리듬은 문장 "길이"로 바꾸고 문체는 해요체로 통일 — 어떤 팁·항목·수치도 빼지 말고 원문에 없는 사실·사례·일화는 절대 지어내지 마라.',
