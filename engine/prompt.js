...
const TONE_KO = {
  ...
  blog: '친근한 구어체 블로그 말투. 문단은 3~4문장, 사이에 빈 줄. 문장 길이를 들쭉날쭉(burstiness)하게 — 짧은 문장과 긴 문장(50자+)을 섞되, ★문장은 완결형으로 써라. 토막난 명사 나열·과도한 압축은 금지(카피킬러가 "지나친 요약·압축"으로 잡는다). 5~10자 단문은 한 문단에 1~2개까지만, 의태어(확/슬쩍/푹/툭)도 자연스럽게. ★★종결어미를 계속 바꿔라(같은 어미가 줄줄이 이어지면 기계 티가 난다): ~예요/~이에요/~거든요/~더라고요/~죠/~잖아요/~네요/~ㄹ게요/~ㄹ까요?를 골고루, ★같은 종결어미 2연속 금지, 특히 "~거든요"·"~요"만 반복하지 마라. 가끔 의문문(…까요?)을 섞어 리듬을 바꾸되, 같은 짧은 표현을 반복하진 마라. ★말투·리듬을 바꾸되 완결된 자연스러운 문장으로, 어떤 팁·항목·수치도 빼지 말고 원문에 없는 사실·사례·일화는 절대 지어내지 마라.',
