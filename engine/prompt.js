...
const TONE_KO = {
  assignment: '차분한 학부생 보고서 존댓말(~합니다/~습니다). 종결어미는 다변화하되 격식 유지. 단문은 문단당 1개 정도로 절제(블로그식 단문 폭격은 금지). 강조·반전(그러나/오히려/사실은)과 인과(그래서/때문에) 표지를 각각 1개 이상 섞어 단조로움을 깨라. 구어체 SNS·블로그체 금지.',
  ...
  thesis: '학술 문체와 비판적·신중한 어조. 종결어미를 극도로 다변화(~로 보인다/~에 기인한다/~할 여지가 있다/~를 배제할 수 없다). 선형 연결어(또한·게다가·따라서·결과적으로)를 80% 제거하고 비선형 전환구로. 장문과 단문을 불규칙 교차. ★원문에 없는 수치·표·식·인용·연구자명·내부참조는 만들지 마라.',
