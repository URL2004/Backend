...
const TONE_KO = {
  assignment: '차분한 학부생 보고서 문체(격식 유지), 종결어미 다변화, 단문은 문단당 1개로 절제(블로그식 단문 폭격 금지). ★문체는 원문을 따른다 — 원문이 평어체(~다/~이다)면 평어체로, 존댓말(~합니다)이면 존댓말로 글 전체를 하나로 통일(섞지 마라). ★카피킬러 대응(지어내지 말 것): (1) 무견해·판단회피 줄이기 — 원문(또는 사용자 메모)에 *이미 있는* 글쓴이의 견해·판단을 hedge("~인 것 같다/~인지 모르겠다") 없이 단정으로 또렷하게 드러내라. ★단, 원문·메모에 없는 새 견해·종합평가·전망("나는 ~라고 본다", "구조적 문제다")을 지어내지 마라 — 근거 없는 견해가 없으면 무견해를 억지로 없애려 하지 말 것. (2) 비인칭·수동 종결("여겨진다/이루어진다/볼 수 있다")을 능동·주체 명시("사람들은 ~한다/나는 ~라고 본다")로 바꿔 관점을 드러내라. (3) 강조·반전(그러나/오히려/사실은)과 인과(그래서/때문에/결국) 접속사로 논점 전환·논리 전개를 살려 기계적 균일성을 깨라. (4) 표준·정형 한자어를 더 자연스러운 우리말로(격식은 유지). (5) 추상 진술은 원문에 실제로 있는 구체 사례·관찰·맥락으로 뒷받침하라 — 단 원문에 없는 사례·수치·출처·일화는 절대 지어내지 마라. 구어체 SNS·블로그체 금지.',
  blog: '친근한 구어체 블로그 말투(~해요/~더라고요/~거든요/~죠). 문단은 3문장 이하로 짧게, 사이에 빈 줄. 5~10자 단문을 가끔(문단당 1개 이하) 섞고, 의태어(확/슬쩍/푹/툭)도 자연스럽게. ★말투·리듬만 바꾸고 어떤 팁·항목·수치도 빼지 마라. ★★전체 분량은 원문 수준을 유지하라 — 대화체로 바꾸되 말을 늘리지 마라(원문에 없는 군더더기·부연·추임새 남발로 길이를 부풀리지 말 것, 원문의 115%를 넘기지 마라).',
...
function buildSystemPrompt(mode = 'assignment', lang = 'ko', { speakerType = 'individual', lengthPolicy, userNotes = '', register = 'mixed' } = {}) {
  ...
  return [
    ...
    '',
    '탐지기 우회(위 FLOOR를 어기지 않는 선에서 *공격적으로* 적용 — 카피킬러는 구조 균일성·비인칭을 직격한다):',
    '· ★★최우선: 비인칭·수동 → 능동·주체(카피킬러 "간접화법·비인칭"이 가장 많이 잡는 신호). 거의 모든 문장에서 "누가/무엇이"를 드러내라. "~된다/~여겨진다/~이루어진다/~할 수 있다/~되곤 한다/~게 된다/~기 마련이다/~는 법이다" 같은 비인칭·수동·정형 종결을 주체 명시 능동으로 바꿔라. 주어가 비면 기업/경영자/조직/사람들/시장/우리 등 글 맥락에 이미 있는 주체를 세워라(없는 주체·사실 창작은 금지). 예) "효율성에 큰 비중을 두었다"→"기업들은 효율성에 매달렸다", "성과가 나타난다"→"몇 년이 지나서야 성과가 얼굴을 내민다", "중요하게 여겨진다"→"경영자들은 이걸 핵심으로 본다", "판단을 요구받는다"→"경영자는 더 복잡한 판단을 떠안는다". 추상 일반론 문장일수록 더 적극적으로 주체를 박아라.',
