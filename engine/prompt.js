...
const TONE_KO = {
  assignment: '차분한 학부생 보고서 존댓말(~합니다/~습니다), 종결어미 다변화·격식 유지, 단문은 문단당 1개로 절제(블로그식 단문 폭격 금지). ★카피킬러 대응(지어내지 말 것): (1) 무견해·판단회피 줄이기 — 원문(또는 사용자 메모)에 *이미 있는* 글쓴이의 견해·판단을 hedge("~인 것 같다/~인지 모르겠다") 없이 단정으로 또렷하게 드러내라. ★단, 원문·메모에 없는 새 견해·종합평가·전망("나는 ~라고 본다", "구조적 문제다")을 지어내지 마라 — 근거 없는 견해가 없으면 무견해를 억지로 없애려 하지 말 것. (2) 비인칭·수동 종결("여겨진다/이루어진다/볼 수 있다")을 능동·주체 명시("사람들은 ~한다/나는 ~라고 본다")로 바꿔 관점을 드러내라. (3) 강조·반전(그러나/오히려/사실은)과 인과(그래서/때문에/결국) 접속사로 논점 전환·논리 전개를 살려 기계적 균일성을 깨라. (4) 표준·정형 한자어를 더 자연스러운 우리말로(격식은 유지). (5) 추상 진술은 원문에 실제로 있는 구체 사례·관찰·맥락으로 뒷받침하라 — 단 원문에 없는 사례·수치·출처·일화는 절대 지어내지 마라. 구어체 SNS·블로그체 금지.',
