...
function lineBefore(text, newlineStart) {
  const start = text.lastIndexOf('\n', Math.max(0, newlineStart - 1)) + 1;
  return text.slice(start, newlineStart).trim();
}

function lineAfter(text, newlineEnd) {
  const end = text.indexOf('\n', newlineEnd);
  return text.slice(newlineEnd, end < 0 ? text.length : end).trim();
}
...
function isStructureLine(line) {
  ...
  if (/^(?:제\s*)?\d+\s*(?:장|절|항)\b/.test(s)) return true;
  if (/^\d+[.)]\s+\S/.test(s)) return true;
  if (/^[가-힣][.)]\s+\S/.test(s)) return true;
  if (/^(?:서론|본론|결론|참고문헌|요약|문제점|개정안|개선안|제안|분석|의의|한계|배경|목적)\s*$/.test(s)) return true;
  if (s.length <= 80 && /(?:중심으로|관련하여|분석|제시|검토|방안|문제점|개정안|개선안|결론)\s*$/.test(s)) return true;
  return false;
}

function isLogicalSingleNewlineBoundary(text, newlineStart, newlineEnd) {
  const before = lineBefore(text, newlineStart);
  const after = lineAfter(text, newlineEnd);
  if (!before || !after) return false;
  return isStructureLine(before) || isStructureLine(after);
}

// 문단 경계(\n{2,})와 보고서형 구조 단일 개행으로 분할.
// 각 청크는 원본 charRange[start,end)와 뒤따르는 구분자(sep)를 보존.
...
function splitChunks(text) {
  ...
  while ((m = re.exec(t)) !== null) {
    const shouldSplit = m[0].length >= 2 || isLogicalSingleNewlineBoundary(t, m.index, m.index + m[0].length);
