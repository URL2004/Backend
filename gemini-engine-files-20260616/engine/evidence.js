...
// ★ 운영 기본은 Anthropic web_search. Gemini 로컬 테스트에서는 Google Search grounding만 사용한다.
//   LLM_BACKEND=gemini일 때 Anthropic/Claude fallback은 금지한다.
...
function claudeFallbackEnabled() {
  if (usingGeminiBackend()) return false;
  ...
  if (usingGeminiBackend()) {
    if (!geminiSearchEnabled()) {
      return {
        ...
        usage: {},
        _provider: 'gemini-disabled'
      };
    }
    return llmGeminiWebSearch({ system, user, signal, maxTokens });
  }
