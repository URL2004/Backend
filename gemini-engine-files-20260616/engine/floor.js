...
function formalHumanJudgmentMode(mode) {
  if (!(mode === 'assignment' || mode === 'thesis' || mode === 'formal')) return false;
  if (process.env.FORMAL_HUMAN === '1' || process.env.ASSIGNMENT_B7 === '1') return true;
  try {
    return require('../llm/profile').geminiFormalHumanJudgment(mode, true);
  } catch {
    return false;
  }
}
...
function buildFloorDirective(povSeed, optIn, mode = 'assignment') {
  const blocks = [];
  blocks.push([
  ...
  if (isSpeakerGateClosed(povSeed, optIn)) {
    ...
    if (formalHumanJudgmentMode(mode)) {
      blocks.push([
        '[화자 보존 — 필자 판단 허용]',
...
function isStructureFragment(s) {
  const raw = (s || '').trim();
  if (!raw) return false;
...
function measureParagraphCollapse(rawText, outputText) {
  const rawParas = String(rawText || '').split(/\n{2,}/).map(p => p.trim()).filter(Boolean).length;
  const outParas = String(outputText || '').split(/\n{2,}/).map(p => p.trim()).filter(Boolean).length;
  const collapsed = rawParas >= 3 && outParas <= Math.max(1, rawParas - 3);
  return { rawParas, outParas, collapsed };
}
function repetitionOverage(rep, base) {
  const b = base || {};
  const over = {
    count: Math.max(0, (rep.count || 0) - (b.count || 0)),
    fuzzyCount: Math.max(0, (rep.fuzzyCount || 0) - (b.fuzzyCount || 0)),
    shortFragCount: Math.max(0, (rep.shortFragCount || 0) - (b.shortFragCount || 0))
  };
  over.total = over.count + over.fuzzyCount + over.shortFragCount;
  over.maxRepeat = rep.maxRepeat || 1;
  return over;
}
function repetitionDetail(rep, base) {
  const over = repetitionOverage(rep, base);
  const sample = (rep.exactItems || []).map(x => x.sample).filter(Boolean).slice(0, 2).join(' | ');
  return `완전중복 ${over.count}건·근접중복 ${over.fuzzyCount}건·짧은조각 ${over.shortFragCount || 0}건${sample ? ` — ${sample}` : ''}`;
}
function measureRepetition(text) {
  const sents = (text || '').split(/(?<=[.!?。])\s+|\n+/).map(s => s.trim())
    .filter(s => s.replace(/\s+/g, '').length >= 15 && !isStructureFragment(s));
  // (1) exact — 정규화 동일 문장 2회+
  const seen = new Map();
  for (const s of sents) {
    const key = normSent(s);
    const prev = seen.get(key) || { count: 0, sample: s.slice(0, 120) };
    prev.count += 1;
    seen.set(key, prev);
  }
  const repeated = [];
  ...
  for (const [, item] of seen) {
    if (item.count >= 2) {
      repeated.push(item.count);
      exactItems.push({ count: item.count, sample: item.sample });
    }
  }
  ...
  for (const raw of (text || '').split(/(?<=[.!?。])\s+|\n+/).map(x => x.trim())) {
    if (isStructureFragment(raw)) continue;
    const s = normSent(raw);
  ...
  return {
    ...
    exactItems: exactItems.slice(0, 5),
...
const META_NOTE_RE = /(메모\s*:|밝힙니다|지시에\s*따라|삽입하지\s*않|본문만\s*출력|위\s*지침|요청하신|원장[은는이가]\s|승인\s*근거|added_claim|distortion|claim\s*ledger|\bREWRITE\b|\bSOURCE\b|\bBODY\b|output\s+only|rewritten\s+sentence|range\s+expression|this\s+still|no\s+commentary|수정\s*문장\s*[:：]|#{1,6}\s*판정|#{1,6}\s*근거)/i;
...
function collectFloorViolations({ result, rawText, povSeed, optIn, mode, position = 'whole', chunkLevel = false, allowedExtra = '', anchors = false }) {
  ...
  const formalHuman = formalHumanJudgmentMode(mode);
  ...
  const paraCollapse = measureParagraphCollapse(rawText, out);
  if (!chunkLevel && (mode === 'assignment' || mode === 'thesis') && paraCollapse.collapsed) {
    v.push({ type: 'paragraph_collapse', detail: `${paraCollapse.rawParas}→${paraCollapse.outParas}`,
      fix: `원문은 ${paraCollapse.rawParas}개 문단인데 출력이 ${paraCollapse.outParas}개 문단으로 압축됐다. 원문의 문단 수와 순서를 되살려 각 문단의 역할을 유지하라. 새 사실 추가 금지, 문단 통합 금지.` });
  }
  // 사실 증발 (숫자 증발 §3.1)
  ...
  const baseRep = measureRepetition(rawText);
  const repOver = repetitionOverage(rep, baseRep);
  if (repOver.total >= 1) {
    v.push({ type: 'repetition', detail: repetitionDetail(rep, baseRep),
...
function buildFloorReport({ result, rawText, mode, povSeed, optIn, allowedExtra = '', anchors = false }) {
  ...
  const paraCollapse = measureParagraphCollapse(rawText, out);
  if ((mode === 'assignment' || mode === 'thesis') && paraCollapse.collapsed) {
    criticals.push({ gate: 'paragraph_collapse', detail: `${paraCollapse.rawParas}→${paraCollapse.outParas}` });
  }
  // ★ 과제 자연체(FORMAL_HUMAN, 격식): 필자 1인칭 판단 허용 → pov_inject 제외(experience_novelty=일화는 위에서 계속 critical).
  const _fhReport = formalHumanJudgmentMode(mode);
  if (!optIn && !_fhReport && drift.introducedFirstPerson) criticals.push({ gate: 'pov_inject', detail: drift.output_fp_singular });
  if (repOver.total) criticals.push({ gate: 'repetition', detail: `exact ${repOver.count}·fuzzy ${repOver.fuzzyCount}·short ${repOver.shortFragCount || 0}${(rep.exactItems || []).length ? ` — ${(rep.exactItems || []).map(x => x.sample).slice(0, 2).join(' | ')}` : ''}` });
  ...
  return {
    ...
    metrics: { lengthRatio: len.ratio, novelty: nov.count, lostFacts: lost.count, repetition: repOver.total,
