...
· 출처 표지는 필요할 때만 짧게 쓰고 반복하지 마라. 근거는 "~에 따르면/~라고 답했다" 나열이 아니라 수치·조건·행위자가 보이는 문장으로 논점 안에 넣어라.
...
· 개인 경험·일화는 금지한다. 그러나 비인칭 설명문만 유지하지 말고, 원문에 있는 근거와 행위자·조건·판단이 드러나는 문장으로 쓴다. 새 사실·새 경험·새 수치·새 기관명은 만들지 마라.
...
function measureArtifacts(text, genre = 'news_explainer') {
  ...
  const v2 = gf.genreRiskScore(text);
  return { ...p, target, badFrameCount: p.badFrame.hits.length, genreRiskScore: v2.score, genreRiskDetail: v2.detail };
}
...
const SKELETONS = {
  ...
  news_article_style: {
    ...
    slots: [
      ...
      { role: 'voices', goal: '조사·기관·연구는 출처 표지를 반복하지 말고 수치·행위자·조건이 보이는 문장으로 전달하라.', w: 1.2 },
...
· 간접·비인칭 해설문 틀: "~에 따르면/~라고 답했다/~로 나타났다" 반복, "~라고 할 수 있다", "~로 볼 수 있다", "~로 해석된다", "이 결과는 보여준다" 남발
...
${anchors}[문체 — 사람 칼럼의 결]
· 한다체. 괄호 삽입구(부연·연도·단서)를 자연스럽게(1000자당 4~8개). 출처 표지는 필요한 곳에만 짧게 두고, 근거는 수치·행위자·조건이 보이는 문장으로 논점에 붙여라.
