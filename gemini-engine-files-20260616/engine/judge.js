...
async function judgeAndRepair(rawText, outputText, { lang = 'ko', signal, maxRounds = 2, allowedExtra = '', approvedFacts = '', mode = '' } = {}) {
  ...
  let _mode = (process.env.FORMAL_HUMAN === '1' || process.env.ASSIGNMENT_B7 === '1') ? 'assignment' : '';
  try {
    if (!_mode && require('../llm/profile').geminiFormalHumanJudgment(mode, true)) _mode = mode;
  } catch {}
