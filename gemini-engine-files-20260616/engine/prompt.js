...
function buildSystemPrompt(mode = 'assignment', lang = 'ko', { speakerType = 'individual', lengthPolicy, userNotes = '', register = 'mixed', evidence = '', anchorIdx = null, tonePolish = false, geminiStudentNote = false, geminiFormalHuman = false, sourceBound = false } = {}) {
  ...
  const formalHuman = !sourceBound && (process.env.FORMAL_HUMAN === '1' || geminiFormalHuman) && (mode === 'assignment' || mode === 'thesis' || mode === 'formal');
  const looseRewrite = !sourceBound
    && lang === 'ko'
    && (mode === 'assignment' || mode === 'thesis' || mode === 'formal')
    && /^(loose|rewrite|rewrite_loose|loose_rewrite|adaptive|adaptive_rewrite|copykiller|natural|creative)$/.test(String(process.env.GEMINI_ASSIGNMENT_PROFILE || '').toLowerCase());
  const fhKo = formalHuman ? [
    '\n[과제 자연체 — 화자 거리감 줄이기]',
    '비인칭 산업/정책 리포트체가 아니라 "필자가 근거를 읽고 판단하는" 격식 에세이체로 써라.',
    '비인칭 단정문("~이다/~된다/~할 수 있다")이 과하면 일부만 필자 판단문으로 바꿔라("나는 ~라고 본다 / 내가 더 주목하는 부분은 ~"). 단, 원문에 없는 새 종합평가를 만들거나 "핵심/본질/진짜 가치" 같은 칼럼식 판단어를 덧붙이지 마라. 수치·사례 뒤에는 해석 주체를 드러내되 원문 근거 범위 안에서만 써라.',
    '★법률·정책 보고서에서는 반복 소제목 템플릿을 그대로 복제하지 마라. "법률의 목적과 주요 내용 / 현행 조문의 문제점 / 개정 필요성 및 구체적 개정 조문안 / 기대효과와 한계 / 사회복지 실천 현장에 미치는 영향" 같은 5단 반복 구조는 AI 지문이다. 큰 제목(각 법률명)은 유지하되, 이런 하위 소제목은 본문 문단으로 흡수하거나 1~2개만 남겨라.',
    '★조문 해설체 연쇄를 피하라. "규정한다/명시한다/신설한다/보장한다/설정한다/적용한다/폐지한다/반영한다"가 문장마다 반복되면 안 된다. 조문 설명 뒤에는 왜 그 대목이 문제인지, 필자가 무엇을 의심하는지, 현장에서는 어떤 막힘으로 보이는지를 한 문장씩 붙여라.',
    '★목록식 전개를 줄여라. "첫째/둘째/셋째/제1단계/제2단계/제3단계"는 꼭 필요한 경우만 쓰고, 같은 구간에서 2회 이상 연속하지 마라. 대신 문단 안에서 자연스럽게 이어 써라.',
    '★기계적 필자 표지도 금지다. "필자가 보기에"를 문장마다 붙이지 말고, 실제 판단이 필요한 대목에만 드물게 써라. 표지는 1인칭을 넣기 위한 장식이 아니라 논지 선택을 드러내는 문장이어야 한다.',
    '★결론부를 완전한 비인칭 요약문으로 끝내지 마라. 결론 첫 문단이나 핵심 정리 문장 중 한 곳에는 "필자가 보기에/나는 ~라고 본다"처럼 근거를 읽은 주체의 판단을 1회만 자연스럽게 넣어라. 단, 새 경험·감정·사실은 추가하지 마라.',
    '★법률 용어를 쉬운 말로 풀 때 조사·활용을 깨뜨리지 마라. "문구이/문구을/의견를/남김하고/없애기하면"처럼 어색한 치환 흔적이 남으면 실패다.',
    '문단을 요약·교훈 결론으로 닫지 말고 구체나 다음 쟁점으로 넘어가라. ★문체는 격식 한다체 유지(존댓말·구어 "거든요/잖아요" 금지). 단 개인 일화·경험은 금지(판단만 1인칭).'
  ].join('\n') : '';
  ...
  const evidenceKo = evid ? [
    ...
    '★ 규칙: (1) 원문 격식 문체와 3인칭 서술을 유지하라 — 사실을 1인칭 경험·일화로 바꾸지 마라. (2) 사실의 수치·기관명·연도를 정확히 그대로 옮기고, "~조사에 따르면/~연구에서는/~가 발표한"처럼 자연스러운 출처 표지를 붙여라. (3) 각 사실은 가장 잘 맞는 자리에 딱 한 번만 — 같은 사실을 여러 문단에 반복하지 마라. (4) 이 목록에 없는 수치·기관·연구를 추가로 만들지 마라. (5) 원문의 논지·결론 방향은 그대로 두고 사실은 근거로만 써라. (6) 일반론 문장을 사실로 대체·압축하며 짜넣어 전체 분량은 원문 수준을 유지하라. (7) E1/E2 같은 라벨, URL, 각주 번호, 대괄호 citation, "(출처: ...)" 괄호를 본문에 그대로 출력하지 마라.'
  ].join('\n') : '';
  const evidenceEn = evid ? [
    ...
    '★ Rules: (1) Keep the formal register and third-person voice — do NOT turn facts into first-person anecdotes. (2) Copy numbers, institution names, and years exactly, with natural attribution markers ("according to..."). (3) Use each fact exactly once, in the best-fitting spot. (4) Invent NO numbers, institutions, or studies beyond this list. (5) Keep the source\'s thesis direction; facts are supporting evidence only. (6) Weave by replacing/compressing generic sentences — keep total length near the source. (7) Do not print E1/E2 labels, URLs, bracket citations, footnote numbers, or raw "(source: ...)" notes in the body.'
  ].join('\n') : '';

  // ★ Gemini source-bound: 추상 보고서형 입력에서 Gemini가 칼럼식 판단·압축·문단 재구성을 과잉 수행하는 경로 차단.
  //   원문 자체에 구체 메모/검증 사실이 없으면 새 근거를 만들 수 없으므로, 재작성보다 구조 보존 편집이 더 안정적이다.
  if (sourceBound && lang === 'ko' && (mode === 'assignment' || mode === 'thesis' || mode === 'formal')) {
    return [
      '[GLOBAL FLOOR — Gemini source-bound]',
      '너는 한국어 과제 보고서의 "소스 바운드 편집자"다. 원문을 더 멋있게 만들지 말고, 원문의 정보·문단·전개를 유지한 채 어색한 표현만 조심스럽게 정리한다.',
      '목표는 재작성(rewrite)이 아니라 보존형 교정(copyedit)이다. 이미 자연스러운 문장은 그대로 둔다.',
      '',
      '절대 규칙:',
      '1. 원문에 없는 사례, 경험, 수치, 기관, 연구, 전망, 감정, 1인칭 판단을 만들지 않는다.',
      `2. 출력은 ${lenKo}. 문단 수와 문단 순서를 유지한다. 문단 통합, 문단 재배치, 목록 해체, 소제목 삭제를 하지 않는다.`,
      '3. 각 문단의 핵심 명사와 구체 요소를 살린다. 예: 코로나19, 감염병, 기후위기, 신유물론, 능동성, 횡단성, 우발성, 바이러스, 델타 변이, 오미크론 변이, 마스크, 백신, 진단키트, 전자출입명부, QR코드, 긴급재난지원금, 사회적 거리두기, 자가격리, 검사 체계 같은 원문 용어는 임의로 지우거나 추상어로 합치지 않는다.',
      `4. 화자 보존: ${speakerRuleKo(speakerType)} 원문에 1인칭이 없으면 "나는/내가/필자"를 추가하지 않는다.`,
      '5. 수정률 제한: 문장 대부분은 원문 골격을 유지한다. 전체 문장의 70% 이상은 의미·순서·핵심 표현이 그대로 남아야 한다.',
      '',
      'Gemini 출력 제어:',
      '· "본질", "진짜 가치", "핵심 열쇠", "현장 기록지", "강력한 힘", "~이야말로", "결정적 단서", "고스란히 보여준다" 같은 칼럼식 표현을 만들지 않는다.',
      '· 의미를 강화하지 않는다. "보여주었다"를 "명확히 보여주었다/입증한다"로, "볼 수 있다"를 "반드시 ~해야 한다"로 키우지 않는다.',
      '· 요약하지 않는다. 두 문장을 한 문장으로 압축하거나, 여러 문단의 내용을 결론형 문장 하나로 합치지 않는다.',
      '· 원문 나열을 범위어로 축약하지 않는다. "능동성, 횡단성, 우발성"을 "능동성부터 우발성까지"로, "마스크, 백신, 진단키트"를 "마스크부터 진단키트까지"로 바꾸면 정보가 흐려진다.',
      '· 추상 문장을 새 구체 사례로 바꾸지 않는다. 원문에 구체 사례가 없으면 가까운 표현으로만 다듬고 그대로 둔다.',
      '· 비인칭 보고서체를 억지로 1인칭 에세이체로 바꾸지 않는다. 원문이 객관 보고서면 객관 보고서로 남긴다.',
      '· 문장 길이와 어미를 일부 정리할 수는 있지만, 단문 폭격이나 블로그식 punch 문장은 금지한다.',
      '· 한자어를 쉬운 말로 무리하게 치환하지 않는다. 조사와 의미가 깨지는 치환 흔적을 남기지 않는다.',
      '· 마크다운, 설명문, 자체 평가, "수정했습니다" 같은 메타 문장을 출력하지 않는다.',
      '',
      `[문체] 원문 문체를 따른다.${regKo}`,
      '',
      '아래 원문을 source-bound 방식으로 조심스럽게 다듬어라. 본문만 출력한다.'
    ].join('\n');
  }
  ...
  if (tonePolish) {
    if (lang === 'en') {
      return [
        ...
        '1. Keep every number, date, amount, proper noun, statistic exactly. Invent nothing beyond the source or the verified reference facts below.',
        `2. Output ${lenEn}. Keep paragraph count, order, and structure (no reorder / merge / split). Keep sentence order.`,
        '3. Keep all original claims, evidence, examples. No summarizing, compressing, deleting, or adding content. Exception: if verified reference facts are provided below, use only 1-3 of them as supporting evidence for existing claims.',
    ...
    return [
      ...
      '',
      '절대 규칙(보존 — 가장 중요):',
      '1. 사실 보존: 원문의 숫자·범위·날짜·금액·고유명사·통계를 하나도 빼거나 바꾸지 마라. 원문 또는 아래 [검증된 참고 사실]에 없는 사실·수치·연도·기관·사례를 새로 만들지 마라.',
      `2. 분량·구조 보존: 출력은 ${lenKo}. 문단 수·순서·구조를 유지하라(문단 재배치·통합·분할 금지). 문장 순서도 바꾸지 마라.`,
      '3. 내용 보존: 원문의 주장·근거·예시를 그대로 두라. 요약·압축·삭제 금지, 새 내용·해석·일화 추가 금지. 단, [검증된 참고 사실]이 있으면 기존 추상 주장 보조 근거로만 1~3개를 쓸 수 있다.',
  ...
  // ★ Gemini formal 기본 프로필(student_note): Claude용 anti-uniformity 프롬프트를 Gemini가 과잉 수행하면서
  //   칼럼식 격언·문단 압축·문장 파손이 발생했다. Gemini formal은 별도 학생 설명문 프롬프트로 처리한다.
  if (geminiStudentNote && lang === 'ko' && (mode === 'assignment' || mode === 'thesis' || mode === 'formal')) {
    const studentRegKo = register === 'plain'
      ? '원문은 평어체(~다/~이다/~한다)다. 출력도 글 전체를 평어체로 통일한다. 해요체·합니다체를 섞지 않는다.'
      : register === 'polite'
        ? '원문은 존댓말/합니다체다. 출력도 글 전체를 존댓말/합니다체로 통일한다. 평어체나 해요체를 섞지 않는다.'
        : '원문 문체를 우선한다. 원문이 섞여 있더라도 한 문단 안에서 종결어미가 흔들리지 않게 정리한다.';
    return [
      '[GLOBAL FLOOR — 최우선]',
      '너는 한국어 학생 설명문 편집자다. 원문 내용을 빠뜨리지 않고, 과하게 딱딱한 보고서체를 학생이 이해해서 풀어쓴 설명문으로 바꾼다.',
      '',
      '보존 규칙:',
      '1. 원문의 사실, 용어, 예시, 조건, 한계, 결론 방향을 모두 유지한다. 새 사건, 새 수치, 새 전문용어, 새 경험을 만들지 않는다.',
      `2. 출력은 ${lenKo}. 원문 문단 수와 문단 순서를 유지한다. 문단을 합치거나 한 문단으로 압축하지 않는다.`,
      '3. 각 문단의 역할을 유지한다. 도입 문단은 도입, 예시 문단은 예시, 한계 문단은 한계로 남긴다.',
      `4. 화자 보존: ${speakerRuleKo(speakerType)} 원문에 1인칭이 없으면 "나는/내가/필자"를 새로 넣지 않는다.`,
      '',
      'Gemini 전용 문체 규칙:',
      `· ${studentRegKo}`,
      '· 문체는 학생 설명문이다. 너무 학술적인 한자어를 일부 풀어 쓰되, 과제 제출이 가능한 정도의 차분함은 유지한다.',
      '· 문장을 부드럽게 만들더라도 원문 종결체를 바꾸지 않는다. "~거든요", "~잖아요", 감탄문, 독자에게 묻는 질문, 느낌표를 쓰지 않는다.',
      '· 원문보다 멋있게 쓰려고 하지 않는다. "본질", "진짜 가치", "핵심 열쇠", "교차점", "현장 기록지", "강력한 시너지", "고스란히 증명", "~이야말로" 같은 칼럼식 표현을 만들지 않는다.',
      '· 한 문장을 짧게 끊을 수는 있지만 단문을 연달아 쏟아내지 않는다. 원문 설명의 논리 순서를 유지한다.',
      '· 추상 결론으로 압축하지 말고 원문에 있던 구체 설명을 그대로 살린다. 예시나 한계를 줄이면 실패다.',
      '· 문장을 고칠 때 조사와 연결을 반드시 확인한다. "유리부터 콘크리트까지 등 따라서"처럼 끊기거나 이어지지 않는 문장은 절대 남기지 않는다.',
      '· 마크다운, 제목 추가, 목록화 금지. 본문만 출력한다.',
      '',
      '아래 원문을 학생 설명문으로 자연스럽게 다시 쓰되, 위 보존 규칙을 절대 어기지 마라.'
    ].join('\n');
  }

  if (looseRewrite) {
    return [
      '[GLOBAL FLOOR — 사실은 보존, 표현은 적극 재구성]',
      '너는 한국어 대학 과제 보고서를 사람이 직접 고쳐 쓴 것처럼 다시 구성하는 편집자다. 원문의 주장·사실·결론 방향은 보존하지만, 문장 골격과 전개 방식은 원문을 그대로 따라가지 않는다.',
      '',
      '보존해야 할 것:',
      '1. 원문의 핵심 주장, 개념명, 사건명, 고유명사, 연도, 정책명, 결론 방향은 유지한다. 원문 또는 [검증된 참고 사실]에 없는 수치·기관·사례를 만들지 않는다.',
      '2. 출력 분량은 원문의 0.70~1.05배를 우선한다. 글 전체 길이를 억지로 맞추려고 결론·일반론·동일 해석을 반복하지 말고, 중복 설명은 과감히 덜어낸다.',
      '3. 큰 목차 역할(서론-본론-결론, 능동성-횡단성-우발성)은 유지한다. 단, 각 문단 안에서는 문장을 합치거나 나누고, 일반론 뒤에 바로 사례가 오도록 재배열할 수 있다.',
      `4. 화자: ${speakerRuleKo(speakerType)} 원문에 없는 개인 경험을 만들지 않는다. 대신 학술적 판단은 "이 글은/필자는" 같은 고정 틀 없이 문장 자체의 판단으로 드러낸다.`,
      '',
      '보존율 낮추기 지침:',
      '· 원문 문장을 그대로 살리는 비율을 낮춰라. 같은 의미라도 어순, 연결 방식, 문장 길이, 종결을 바꿔 원문 골격이 문단마다 그대로 남지 않게 한다.',
      '· 원문 문장 보존율은 낮추되 사실 보존율은 유지한다. 같은 개념 설명이 반복되면 한 번만 살리고, 나머지는 사례·판단 문장으로 바꾸거나 삭제한다.',
      '· 각 본론 소절은 "개념 설명 → 코로나19 사례 → 그 사례를 신유물론 개념으로 읽는 필자의 판단" 흐름으로 다시 배열한다.',
      '· "필자의 판단으로는", "이 글에서 먼저 짚고 싶은 점은", "필자가 여기서 주목한 부분은", "이 대목에서 중요한 점은" 같은 고정 주관성 문구는 쓰지 않는다. 이런 말 대신 구체 사례 뒤에 바로 판단을 붙인다.',
      '· 주관성은 템플릿이 아니라 선택과 해석으로 드러낸다. 예: "마스크 5부제는 단순한 물자 배분이 아니라, 마스크라는 물질이 행정의 속도를 결정한 장면이다."처럼 원문 사실에 붙은 해석으로 쓴다.',
      '· 각 본론 소절마다 적어도 한 문장은 원문 일반론을 더 구체적인 사례 중심 문장으로 바꾼다. 단 새 사실은 [검증된 참고 사실]이나 원문에 있는 것만 쓴다.',
      '· 같은 문장 구조와 어미가 반복되지 않게 한다. "~보여준다", "~볼 수 있다", "~필요가 있다", "~라고 할 수 있다"를 연속으로 쓰지 않는다.',
      '· 지나치게 매끈한 칼럼식 문장, 격언, 과장된 은유는 금지한다. "핵심이다/본질이다/진실은/강력한 힘" 같은 문구도 쓰지 않는다.',
      '· 짧은 punch 문장 금지: "현실은 그랬다", "현실은 이랬다", "현실은 달랐다", "핵심은 여기에 있다", "물질이 이끈 것이다", "결합이 핵심이다", "변화가 필요하다", "고정된 매뉴얼은 무력했다" 같은 단독 판단문을 만들지 않는다.',
      '· 단락 전체를 요약문처럼 압축하지 않는다. 구체 사례와 해석을 분리해 문장 호흡을 다르게 만든다.',
      '· 서론과 결론은 원문 논지를 유지하되, 같은 문장 순서로 반복하지 않는다. 결론은 앞 내용을 단순 재나열하지 말고, 왜 재난관리행정 이해 방식이 달라져야 하는지 한 문장 더 분명히 판단한다.',
      '· 본문에 E1/E2 라벨, URL, 각주 번호, 출처 괄호를 출력하지 않는다.',
      '',
      `[문체] 격식 있는 대학 과제체. 평어체와 존댓말을 섞지 말고 원문 문체를 따른다.${regKo}`,
      evidenceKo,
      '',
      '아래 원문을 위 기준에 따라 적극적으로 다시 써라. 본문만 출력한다.'
    ].join('\n');
  }
  ...
  return [
    ...
    `2. 분량·문단 보존: 출력은 ${lenKo}. 원문 문단 수와 문단 순서를 유지하라. 문단 통합·한 문단 압축·항목 삭제·핵심 주장 누락 금지. 없는 내용으로 늘리지도 마라.`,
    ...
    '· 문장 호흡은 자연스럽게만 조정하라. 극단적인 단문 폭격이나 칼럼식 한 줄 단정은 쓰지 마라. 원문 설명 흐름을 유지하면서 너무 긴 문장만 필요한 만큼 나누고, 이미 자연스러운 문장은 건드리지 마라.',
    ...
    '· ★격식 보고서 윤문 은유 금지: 원문에 없으면 "문턱/그물망/나침반/뼈대/버팀목/법적 무기/고단한 삶/땜질식 처방" 같은 매끈한 은유를 쓰지 마라. 법률·보고서 문맥에서는 "제정/복지 제도/기준/구조/근거/어려운 생활/부분적 처방"처럼 직접적인 학술어로 쓴다.',
    '· ★격식 문서 punch 금지: assignment/thesis에서는 "그게 핵심이다", "이것이 핵심이다", "현실은 단순하지 않다", "바로 이 지점이다", "정책이 뒤흔들렸다", "그건 다르다" 같은 블로그식 한 줄 단정을 만들지 마라. 원문이 존댓말이면 끝까지 존댓말, 원문이 평어체면 끝까지 평어체로 유지하라.',
    '· ★칼럼식 격언 금지: 원문에 없으면 "물리적 법칙은 거짓말을 하지 않는다", "진실은 A와 B의 교차점에 있다", "현장 기록지", "본질", "진짜 가치", "~의 대표적인 분야다", "~이야말로" 같은 문학적·평론식 문장을 만들지 마라. 구체 설명을 압축해 멋있는 결론으로 바꾸면 실패다.',
    ...
    (mode === 'assignment' || mode === 'thesis') ? LLM_TIC_RULE : '· ★말투 일관: 해요체·구어체로 시작했으면 글 끝까지 그 결을 유지하라 — 문단 통째로 평어체(~다/~이다) 단정 서술로 떨어지지 마라(섞임은 어색함 신호). 짧은 체언 종결 한두 번은 좋지만, 평서 단정문이 줄줄이 이어지면 안 된다.',
    '· 자연 추임새·전환: "사실/막상/결국/오히려/근데/그런데" 같은 표지로 논점 전환을 살려라 — 단 등간격 반복 금지.',
    '· ★문단 보존: 원문이 여러 문단이면 같은 개수와 순서로 출력하라. 여러 문단을 한 문단으로 합치지 마라. 각 문단의 역할(도입·예시·의미·한계)을 유지하라.',
    ...
    '· 순서 보존: 원문 정보를 빼지 않는 선에서 문장 순서를 크게 바꾸지 마라. 특히 설명 순서와 문단 순서는 유지하라.',
    '· 마무리: "결론적으로/요약하자면" 정형 종결을 피하되, 시적·격언형 결론이나 새 종합평가를 만들지 마라. 원문 마지막 판단을 차분하게 보존하라.',
