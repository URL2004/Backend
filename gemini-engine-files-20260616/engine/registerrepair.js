...
· 이 문단의 비인칭 단정문("~이다/~된다/~할 수 있다/~는 것이다") 중 1~2개를 *필자 판단이 보이는 문장*으로 구조 변형하라.
  "나는/내가/필자가 보기에" 같은 고정 표지를 반복하지 말고, 가능하면 문장 구조로 판단을 드러내라:
  "여기서 먼저 봐야 할 것은 ~다", "이 대목은 ~보다 ~를 더 문제로 만든다", "이 조건에서는 ~가 먼저 흔들린다".
· *어떤 근거를 어떻게 읽는지*를 드러내라(근거 재해석): "자료에 따르면 X. 신호다" → "그 수치에서 더 눈여겨볼 부분은 순위가 아니라 ~다".
...
· ★개인 경험·일화는 절대 금지(제가 ~했을 때/제 친구가). 새 사실·수치·고유명사도 금지. 오직 *기존 논지에 대한 필자의 판단·해석*만 드러내라.
...
async function registerRepairPass(text, rawText, { lang = 'ko', signal, floor, ledger, mode = 'assignment', allowedExtra = '', maxParas = 22 } = {}) {
  ...
  for (const i of order) {
    ...
    let cand = '';
    try { cand = (await llmText({ system, user, signal, maxTokens: 1400, model: HAIKU, task: 'voiceRepair', mode, riskLevel: 'medium', textLength: p.replace(/\s+/g, '').length }) || '').trim(); } catch { continue; }
    if (!cand) continue;
