// llm/router.js - local Gemini super-conservative model router.
...
function chooseGeminiModel({ task, mode, riskLevel, textLength, model, tool }) {
  ...
  if (t === 'ledger' || t === 'judge' || t === 'evidence' || t === 'grounding') return gemini.MODELS.PRO;
  if (mode === 'thesis' && (t === 'rewrite' || t === 'formal')) return gemini.MODELS.PRO;
  if (t === 'rewrite' || t === 'repair' || t === 'polish' || t === 'voiceRepair' || t === 'genre' || t === 'formal') return gemini.MODELS.FLASH;
  if (len > 8000 && t !== 'judge' && t !== 'ledger' && t !== 'evidence') return gemini.MODELS.FLASH;
  if (model === CLAUDE_SONNET && t !== 'rewrite') return gemini.MODELS.FLASH;
...
function usageLogBase(meta, provider, model, startedAt, status, usage, err) {
  return {
    ...
    schemaFormat: usage?.schema_format || null,
    cachedContent: usage?.cached_content || null,
    cacheSource: usage?.cache_source || null,
    cacheKey: usage?.cache_key || null,
    finishReason: usage?.finish_reason || null,
    safetyRatings: Array.isArray(usage?.safety_ratings) ? usage.safety_ratings : undefined,
    groundingMetadataPresent: !!usage?.grounding_metadata_present,
    groundingWebSearchQueries: usage?.grounding_web_search_queries || 0,
...
function mergeUsage(a, b) {
  ...
  return {
    ...
    cache_source: b.cache_source || a.cache_source || null,
    cache_key: b.cache_key || a.cache_key || null
  };
}
