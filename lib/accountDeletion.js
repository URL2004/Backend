...
const USER_SUBCOLLECTIONS = Object.freeze([
  'creditHistory', 'couponHistory', 'creditLots', 'history', 'notifications',
]);
...
async function deleteDetectStabilityRowsForUid(db, uid, options = {}) {
  // Stability rows deliberately omit a raw UID. A domain-separated keyed
  // lookup keeps them erasable without turning the detector cache into a
  // plaintext per-user index.
  const detectStability = require('./detectResultStability');
  const deletionKey = detectStability.deletionKeyForUid(uid, {
    hmacSecret: options.hmacSecret
  });
  detectStability.purgeForUid(uid, { hmacSecret: options.hmacSecret });
  if (!deletionKey) return 0;
  return deleteCollectionRowsForUid(db, 'analyzeRequests', deletionKey, 'stabilityDeletionKey');
}
...
async function deleteUserOperationalData(db, uid) {
  ...
  counts.analyzeRequests += await deleteDetectStabilityRowsForUid(db, uid);
