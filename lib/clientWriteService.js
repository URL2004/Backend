...
const { verifiedBackupInterpretation } = require('./detectHistoryPresentation');
const { verifiedBackupHistoryComparison } = require('./detectHistoryComparison');
...
const CALIBRATION_KEYS = new Set([
  ...
  'rawProbability', 'calibratedProbability', 'sourceProbability', 'sourceCapApplied', 'maxReduction', 'floor', 'factor'
]);
...
function sanitizeCalibration(value) {
  ...
  for (const key of ['applied', 'sourceCapApplied']) {
    if (value[key] != null) {
      if (typeof value[key] !== 'boolean') throw apiError(400, 'INVALID_INPUT', `${key} 값이 올바르지 않아요.`);
      result[key] = value[key];
    }
  }
  const numericLimits = { matchSimilarity: 1, matchLengthRatio: 1, rawProbability: 100,
    calibratedProbability: 100, sourceProbability: 100, maxReduction: 100, floor: 100, factor: 1 };
  for (const [key, max] of Object.entries(numericLimits)) {
    const number = optionalFiniteNumber(value[key], key, { min: 0, max });
    if (number !== undefined) result[key] = number;
  }
  if (Object.hasOwn(value, 'sourceProbability') && value.sourceProbability === null) result.sourceProbability = null;
  return result;
}
...
async function consumeQuota(transaction, ref, { uid, action, nowMs = Date.now(), fieldValue, snapshot }) {
  ...
  if (hourCount >= limit.hourly) {
    throw apiError(429, 'WRITE_QUOTA_EXCEEDED', '잠시 후 다시 시도해 주세요.', {
      quotaAction: action,
      quotaScope: 'hourly',
      quotaCount: hourCount,
      quotaLimit: limit.hourly,
      ...(action.startsWith('account_initialize') ? { grantCredits: SIGNUP_GRANT_CREDITS } : {}),
      retryAfterSec: quotaRetryAfter('hourly', nowMs)
    });
  }
  if (dayCount >= limit.daily) {
    throw apiError(429, 'WRITE_QUOTA_EXCEEDED', '오늘 저장할 수 있는 횟수를 모두 사용했어요.', {
      ...
      quotaScope: 'daily',
      quotaCount: dayCount,
      quotaLimit: limit.daily,
      ...(action.startsWith('account_initialize') ? { grantCredits: SIGNUP_GRANT_CREDITS } : {}),
      retryAfterSec: quotaRetryAfter('daily', nowMs)
    });
  }
  transaction.set(ref, {
    uid,
...
function createClientWriteService({ db, admin, now = () => Date.now() }) {
  ...
  async function initializeAccount({ uid, email, name, signupAttribution, clientPrincipal, metaContext }) {
    ...
    const accountKey = signupCreditAccountKey(safeUid);
    const principalKey = signupCreditPrincipalKey(principal);
    const grantEventRef = db.collection(SIGNUP_CREDIT_EVENT_COLLECTION)
      .doc(signupCreditEventId('grant', accountKey, 'initial_grant_v1'));
    return db.runTransaction(async transaction => {
      ...
      if (metaConversions.enabled() && !(require('../config').ADMIN_UIDS || []).includes(safeUid)) {
        require('./metaOutbox').stageInTransaction(transaction, db, metaConversions.buildCompleteRegistration({
          eventId: metaConversions.stableEventId('sign_up', `${safeUid}|${createdAt}`),
          eventTime: Math.floor(nowMs / 1000), email: safeEmail, externalId: safeUid,
          context: metaContext, clientIp: metaContext?.clientIp, userAgent: metaContext?.userAgent
        }), safeUid, nowMs);
      }
      transaction.set(userRef, {
        ...
        signupCreditGrant: createSignupCreditGrantMetadata({
      ...
      transaction.set(grantEventRef, buildSignupCreditEvent({
        eventType: 'grant',
        accountKey,
        principalKey,
        creditAmount: SIGNUP_GRANT_CREDITS,
        signupCreditsRemaining: SIGNUP_GRANT_CREDITS,
        accountCreditsRemaining: SIGNUP_GRANT_CREDITS,
      ...
      return {
        ...
        credits: SIGNUP_GRANT_CREDITS,
        createdAt,
        quota: { uid: uidQuota, client: ipQuota }
      };
    });
  }
  ...
  async function backupHistory({ uid, requestId, entry }) {
    ...
    if (clean.type === 'detect') {
      const originalInput = entry.inputText ?? '';
      // The ordinary text sanitizer normalizes CRLF and removes controls.
      // A signed interpretation instead binds exact source bytes and offsets.
      // Bound that original string before verification and retain it unchanged
      // only when its server proof is valid.
      if (typeof originalInput !== 'string') throw apiError(400, 'INVALID_INPUT', '원문 형식이 올바르지 않아요.');
      if (originalInput.length > 60000 || Buffer.byteLength(originalInput, 'utf8') > 240000) {
        throw apiError(413, 'PAYLOAD_TOO_LARGE', '원문이 너무 길어요.');
      }
      // Browser prose, confidence, locations and model-source labels cannot
      // establish evidence. Only the UID/source/score-bound server proof can.
      const verified = verifiedBackupInterpretation(uid, {
        interpretation: entry.interpretation, interpretationProof: entry.interpretationProof,
        probability: clean.probability, inputText: originalInput
      });
      const comparison = verifiedBackupHistoryComparison(uid, {
        historyComparison: entry.historyComparison, historyComparisonProof: entry.historyComparisonProof,
        probability: clean.probability, inputText: originalInput
      });
      if (verified || comparison) clean.inputText = originalInput;
      if (comparison) {
        clean.historyComparison = comparison;
        clean.rawProbability = comparison.rawProbability;
      }
      clean.interpretation = verified || buildDetectInterpretation({
        probability: clean.probability, probSource: 'unknown', textLength: clean.inputText.length
      });
      if (Buffer.byteLength(JSON.stringify(clean), 'utf8') > 600000) {
        throw apiError(413, 'PAYLOAD_TOO_LARGE', '저장할 이용 기록이 너무 커요.');
      }
    }
