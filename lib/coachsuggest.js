...
async function generateCoach(text, { signal } = {}) {
  ...
  try {
    ...
    useGpt = gptRuntimeConfig.isGptActive(cfg);
    const call = useGpt
      ? (opts) => gptAnalyze.callGpt({ ...opts, config: cfg })
      : analyze.callClaude;
    data = await call({
  ...
  const r = (useGpt ? gptAnalyze.extractGptResult : analyze.extractClaudeResult)(data, 'return_coach_suggestions') || {};
