...
function normalizeTrackedLot(value) {
  ...
  const bonusCap = Math.max(
    bonusRemaining,
    whole(value.eventBonusCreditsCap ?? value.eventBonusCredits ?? value.promotionalBonusCredits)
  );
  return {
...
function allocateProviderCancellation({
  ...
  if (own && own.id === String(canceledOrderId || '') && remainingTarget > 0 && walletAvailable > 0) {
    // Cancellation revokes the event bonus before refundable paid credits.
