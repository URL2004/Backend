...
function legacyAuthMode(options = {}, env = process.env) {
  const configured = clean(options.legacyMode === undefined ? env.CRON_LEGACY_AUTH_MODE : options.legacyMode).toLowerCase();
  return configured === 'block' ? 'block' : 'shadow';
}
...
function legacyCredentialEntries(req, options = {}) {
  const entries = [];
  if (options.allowBody !== false) entries.push({ source: 'body', value: clean(req && req.body && req.body.internalKey) });
  if (options.allowQuery) entries.push({ source: 'query', value: clean(req && req.query && req.query.key) });
  return entries.filter(entry => entry.value);
}
...
function verifyCronRequest(req, options = {}) {
  ...
  const mode = legacyAuthMode(options);
  const primaryEntries = credentialEntries(req, { ...options, allowBody: false, allowQuery: false });
  const legacyEntries = legacyCredentialEntries(req, options);
  if (mode === 'block' && !primaryEntries.length && legacyEntries.length) {
    return {
      ok: false,
      reason: 'legacy_credential_blocked',
      authSource: legacyEntries.map(entry => entry.source).join('+'),
      hasCredential: true,
      sourceCount: legacyEntries.length,
      credentialConflict: false,
      legacyCredentialUsed: true,
      legacyCredentialPresent: true,
      legacyMode: mode,
      userAgentFamily: uaFamily
    };
  }
  const entries = mode === 'block' ? primaryEntries : [...primaryEntries, ...legacyEntries];
  if (!entries.length) {
    return {
      ok: false,
  ...
  if (distinct.size > 1) {
    return {
      ...
      legacyCredentialUsed: entries.some(entry => entry.source === 'body' || entry.source === 'query'),
      legacyCredentialPresent: legacyEntries.length > 0,
      legacyMode: mode,
      userAgentFamily: uaFamily
    };
  }
...
function authLogFields(result) {
  return {
    ...
    legacyCredentialUsed: !!result.legacyCredentialUsed,
    legacyCredentialPresent: !!result.legacyCredentialPresent,
