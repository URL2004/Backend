...
const VERSION = 'history-calibration-v5-style-score';
...
// The lookup hash ignores whitespace for legacy compatibility; it is not a
// proof of equality. Korean word division can change a subject into an object
// without changing a single letter. Preserve boundaries inside word sequences,
// while allowing line wrapping, repeated spaces and punctuation spacing.
function wordBoundaryKey(text) {
  return String(text || '').normalize('NFKC').replace(/[\u200B-\u200D\uFEFF]/g, '')
    .replace(/。/gu, '.').replace(/、/gu, ',')
    .replace(/\s+/gu, (space, offset, whole) =>
      /[\p{L}\p{N}]/u.test(whole[offset - 1] || '')
        && /[\p{L}\p{N}]/u.test(whole[offset + space.length] || '') ? ' ' : '');
}
...
// Count the same eligible sample units as detection, before whitespace removal.
const APPROX_ABS_MIN_CHARS = 200;
function countSentenceMarks(text) {
  return require('./detectInputDocument').buildDetectInputDocument(text).eligibleSentenceCount;
}

function approximateEligible(normalized, cfg, rawText = normalized) {
  ...
  return normalized.length >= cfg.minApproximateChars
    || (countSentenceMarks(rawText) >= cfg.minApproximateSentences && normalized.length >= APPROX_ABS_MIN_CHARS);
}
...
function nearIntegrityCompatible(left, right) {
  // A 3% document edit can reverse a short claim in a long document. String
  // similarity is not semantic proof: without a new verified server result,
  // allow only equivalent typography, not an unverified lexical rewrite.
  const typography = value => String(value).replace(/。/gu, '.').replace(/、/gu, ',');
  if (typography(left) !== typography(right)) return false;
  const anchors = value => JSON.stringify({
    ...
    protected: require('../engine/textSyntax').syntaxSpans(value)
      .filter(span => span.spanType !== 'parenthetical')
      .map(span => value.slice(span.start, span.end))
  });
...
function approximateMatchMetrics(target, candidate, cfg, targetShingles, raw = {}) {
  ...
  if (!cfg.approximateMatchEnabled
    || !approximateEligible(shorter, cfg, targetLength <= candidateLength ? raw.target : raw.candidate)
  ...
  return {
    ...
    matched: similarity >= cfg.similarityThreshold && nearIntegrityCompatible(target, candidate)
      && (raw.target == null || raw.candidate == null || wordBoundaryKey(raw.target) === wordBoundaryKey(raw.candidate)),
    similarity,
    lengthRatio,
    lengthDeltaRatio
  };
...
async function verifyExactTransformJobMatch({ db, uid, historyId, target, rawTarget }) {
  ...
  if (job.uid !== uid
    ...
    || wordBoundaryKey(provenance.outputText) !== wordBoundaryKey(rawTarget)
...
async function findOwnHumanizedHistoryMatch({ db, uid, text, limit, config: rawConfig }) {
  ...
  const indexedTrusted = indexedDocs.find(doc => {
    const h = doc.data() || {};
    return normalizeText(h.outputText) === target && wordBoundaryKey(h.outputText) === wordBoundaryKey(text)
      && historyLinkIntegrity.verify(uid, h.outputText, h, h.historyLinkIntegrity);
  });
  const docs = indexedTrusted ? [indexedTrusted] : (await query.get()).docs;
  const targetShingles = approximateEligible(target, cfg, text)
  ...
  for (const doc of docs) {
    ...
    if (output === target) {
      if (wordBoundaryKey(h.outputText) !== wordBoundaryKey(text)) continue;
    ...
    const metrics = approximateMatchMetrics(target, output, cfg, targetShingles, { target: text, candidate: h.outputText });
