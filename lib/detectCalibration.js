...
const { verifyHistoryProvenance } = require('./historyService');

const VERSION = 'history-calibration-v3';
...
async function findOwnHumanizedHistoryMatch({ db, uid, text, limit, config: rawConfig }) {
  ...
  if (query && typeof query.select === 'function') {
    query = query.select('type', 'outputText', 'mode', 'createdAt', 'savedBy', 'serverProvenance');
  }
  ...
  for (const doc of snap.docs) {
    ...
    if (h.savedBy !== 'server' || !verifyHistoryProvenance({
      uid,
      type: h.type,
      mode: h.mode,
      outputText: h.outputText
    }, h.serverProvenance)) continue;
    humanizedCount += 1;
