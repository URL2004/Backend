...
const DEFAULT_CONFIG = {
  ...
  //   하나만 넘으면 근사 일치를 시도한다(정확 일치는 100자부터 그대로).
...
function sourceProbabilityOf(record = {}) {
  const n = Number(record.sourceProbability);
  ...
  for (const doc of snap.docs) {
    ...
    if (output === target) {
      ...
      return {
        ...
        sourceProbability: sourceProbabilityOf(h)
      };
    }
...
async function applyHistoryCalibration({ db, uid, text, probability, logger, route }) {
  ...
  const sourceCap = cfg.sourceCapEnabled && match.sourceProbability != null ? match.sourceProbability : null;
