...
async function findOwnHumanizedHistoryMatch({ db, uid, text, limit, config: rawConfig }) {
  ...
  if (query && typeof query.select === 'function') {
    query = query.select('type', 'outputText', 'mode', 'createdAt', 'historyLinkIntegrity');
  }
  ...
  for (const doc of snap.docs) {
    ...
    if (!output) continue;
    // users/{uid}/history는 과거 클라이언트 쓰기를 허용했던 컬렉션이다.
    // 신규 보정 근거는 서버만 만들 수 있는 UID 결합 HMAC이 있어야 신뢰한다.
    // 구 레코드는 관리자 이력 화면에서 계속 읽을 수 있지만 감지 점수 보정에는 사용하지 않는다.
    if (!historyLinkIntegrity.verify(uid, h.outputText, h.historyLinkIntegrity)) continue;
    if (output === target) {
      return {
        id: doc.id,
        mode: h.mode || null,
