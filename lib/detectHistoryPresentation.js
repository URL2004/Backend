...
const KEYS = new Set(['version', 'score', 'status', 'band', 'subBand', 'label', 'headline', 'description', 'evidence', 'pattern', 'nextSteps', 'limitations', 'sample', 'assessability', 'scoreKind', 'scoreLabel', 'evidenceDetails']);
...
function boundedInterpretation(value, score) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
      || !['detect-interpretation-v1', 'detect-interpretation-v2', 'detect-interpretation-v3', VERSION].includes(value.version) || value.score !== normalizeScore(score)
      || Object.keys(value).some(key => !KEYS.has(key))) return null;
  if (Object.hasOwn(value, 'scoreKind') && value.scoreKind !== 'ai_style') return null;
  if (Object.hasOwn(value, 'scoreLabel') && value.scoreLabel !== 'AI식 문체 점수') return null;
  if (Object.hasOwn(value, 'evidenceDetails')
      && (!closed(value.evidenceDetails, ['sampleSufficient', 'locatedPatterns', 'statisticalContribution', 'inputIncomplete'])
        || !['sampleSufficient', 'statisticalContribution', 'inputIncomplete'].every(k => typeof value.evidenceDetails[k] === 'boolean')
        || !Number.isSafeInteger(value.evidenceDetails.locatedPatterns)
        || value.evidenceDetails.locatedPatterns < 0 || value.evidenceDetails.locatedPatterns > 10)) return null;
  // Keep v1 descriptors byte-for-byte equivalent for existing signed backups.
  if (value.version === 'detect-interpretation-v1') {
    if (Object.hasOwn(value, 'assessability')) return null;
  } else if (!closed(value.assessability, ['status', 'meaning'])
      || !['unavailable', 'insufficient', 'limited', 'available'].includes(value.assessability.status)
      || typeof value.assessability.meaning !== 'string' || value.assessability.meaning.length > 1000) return null;
  ...
  if (value.pattern !== null) {
    ...
    if (!closed(pattern, ['category', 'label', 'description', 'locationCount', 'scope', 'paragraphIndices'])
        || !CATEGORIES.has(pattern.category) || !['isolated', 'recurring', 'pervasive'].includes(pattern.scope)
...
function storedDetectInterpretation(result = {}, text = '') {
  ...
  return buildDetectInterpretation({
    ...
    inputIncomplete: buildDetectInputDocument(text).inputIncomplete,
    sentenceTotal: report.measuredEvidence?.sentenceTotal ?? report.contentEvidence?.total ?? buildDetectInputDocument(text).eligibleSentenceCount,
    signalEvidence: result.signalEvidence || report.causeAnalysis?.items || [],
    statisticalSupport: require('./detectStatisticalAssist').sanitizeSupport(result.statisticalSupport),
    statisticalReference: require('./detectStatisticalAssist').sanitizeReference(result.statisticalReference || result.detectStatisticalReference),
    calibrationApplied: result.probabilityCalibration?.applied === true,
    preCalibrationProbability: result.rawProbability,
