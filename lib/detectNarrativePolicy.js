...
const DISCLAIMER = 'AI 감지 결과는 문체 패턴을 바탕으로 한 추정치이며, 실제 작성 주체를 확정하지 않습니다.';
...
const BANDS = Object.freeze({
  low: Object.freeze({
    ...
    detail(probability) {
      return `최종 점수 ${probability}%는 낮은 구간입니다. 정돈된 문장이나 반복 표현이 일부 관찰되더라도 전체 판정은 낮은 위험으로 해석해야 합니다.`;
    }
  }),
  moderate: Object.freeze({
    ...
    detail(probability) {
      return `최종 점수 ${probability}%는 중간 구간입니다. 일부 정형적인 문체 특징이 관찰됐지만, 이 결과만으로 작성 주체를 단정하기는 어렵습니다.`;
    }
  }),
  high: Object.freeze({
    level: 'high',
    ...
    detail(probability) {
      return `최종 점수 ${probability}%는 높은 구간입니다. 아래 문체 특징이 점수를 높인 신호로 관찰됐습니다.`;
    }
  })
});
...
function applyDetectNarrativePolicy(result = {}, probabilityOverride) {
  ...
  const signals = cleanSignals(result.signals);
