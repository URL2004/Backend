...
// 구체 앵커·화자 입장은 글 종류마다 "있어야 정상"인 정도가 다르고, 문장이 적으면 비율 자체가 무의미하다.
// 축마다 on(기준 적용) · soft(참고만, '높음' 판정 없음) · off(이 글 종류엔 해당 없음) · sparse(문장 부족)를
// 내려보내고 프런트는 정책대로 그리기만 한다. 점수(AI 티 지수)와는 무관하다 — 원인 설명 전용.
// 기준값(target)은 2026-09-03 88쌍 원문 분포(문서/03_원문-결과-분석/analysis_extract.json)에서 종류별 p50이
// '보통' 근처, p75 이상이 '낮음' 쪽에 놓이도록 잡았다(보고서·논문의 앵커 p75 0.02~0.04, 자소서 화자 입장 p50 0.17).
const RADAR_AXIS_POLICY_VERSION = 'axis-policy-v1';
...
// anchor.metric: 'anchor' = 숫자·연도·고유명사·행동이 함께 있는 문장 비율, 'lived' = 실제 경험 문장 비율.
// target = 이 비율이면 '부족 0'. off인 축은 reason이 화면 문구가 된다.
const AXIS_POLICY_BY_PROFILE = Object.freeze({
  academic_paper: { anchor: { status: 'on', metric: 'anchor', target: 0.12 }, stance: { status: 'off', reason: '논문은 사견보다 근거가 우선이라 화자 입장 축은 보지 않아요' } },
  report_assignment: { anchor: { status: 'on', metric: 'anchor', target: 0.10 }, stance: { status: 'off', reason: '보고서·과제에는 화자 입장이 꼭 필요하지 않아 이 축은 보지 않아요' } },
  long_explainer: { anchor: { status: 'on', metric: 'anchor', target: 0.10 }, stance: { status: 'off', reason: '설명글에는 화자 입장이 꼭 필요하지 않아 이 축은 보지 않아요' } },
  clinical_record: { anchor: { status: 'on', metric: 'anchor', target: 0.12 }, stance: { status: 'off', reason: '기록 문서에는 화자 입장이 필요하지 않아 이 축은 보지 않아요' } },
  legal_contract: { anchor: { status: 'on', metric: 'anchor', target: 0.10 }, stance: { status: 'off', reason: '계약·법률 문서에는 화자 입장이 필요하지 않아 이 축은 보지 않아요' } },
  mail_notice: { anchor: { status: 'soft', metric: 'anchor', target: 0.08 }, stance: { status: 'off', reason: '안내문에는 화자 입장이 필요하지 않아 이 축은 보지 않아요' } },
  marketing: { anchor: { status: 'soft', metric: 'anchor', target: 0.06 }, stance: { status: 'off', reason: '홍보 문구에는 화자 입장이 필요하지 않아 이 축은 보지 않아요' } },
  student_record_teacher: { anchor: { status: 'on', metric: 'anchor', target: 0.10 }, stance: { status: 'off', reason: '교사가 3인칭으로 쓰는 기록이라 화자 입장 축은 보지 않아요' } },
  student_self_assessment: { anchor: { status: 'on', metric: 'lived', target: 0.30 }, stance: { status: 'on', target: 0.20 } },
  resume_application: { anchor: { status: 'on', metric: 'lived', target: 0.30 }, stance: { status: 'on', target: 0.30 } },
  personal_essay: { anchor: { status: 'on', metric: 'lived', target: 0.30 }, stance: { status: 'on', target: 0.25 } },
  review_blog: { anchor: { status: 'on', metric: 'anchor', target: 0.10 }, stance: { status: 'on', target: 0.25 } },
  social: { anchor: { status: 'soft', metric: 'lived', target: 0.20 }, stance: { status: 'on', target: 0.20 } },
  creative: { anchor: { status: 'soft', metric: 'lived', target: 0.20 }, stance: { status: 'soft', target: 0.15 } },
  general: { anchor: { status: 'soft', metric: 'anchor', target: 0.08 }, stance: { status: 'soft', target: 0.15 } },
  unknown: { anchor: { status: 'soft', metric: 'anchor', target: 0.08 }, stance: { status: 'soft', target: 0.15 } }
});
const AXIS_LOW_CONFIDENCE = 0.55;
const AXIS_SOFT_REASON = '글 종류가 확실하지 않아 참고로만 봐요';
const AXIS_SPARSE_REASON = '문장이 적어 재지 않았어요';
const AXIS_SPARSE_ALL_NOTE = '짧은 글은 문체 통계 신호를 잴 수 없어요. 600자쯤(약 8문장)부터 원인 분석이 열려요.';

function resolveRadarAxisPolicy({ profile, confidence, sentenceTotal } = {}) {
  ...
  const conditioned = (axis, spec) => {
    ...
    const soften = spec.status === 'soft' || lowConfidence;
    return { status: soften ? 'soft' : 'on', metric: spec.metric || axis, target: spec.target, reason: soften ? AXIS_SOFT_REASON : null };
  };
  const axes = {
...
// 두 문장의 낱말 LCS에서 "짝이 없는 낱말"이 가장 길게 이어진 구간을 각각 [start,end) 글자 범위로 돌려준다.
//   after 쪽은 새로 쓰인 자리(공개 창), before 쪽은 바뀌는 자리(원문에 표시).
function longestUnmatchedRun(spans, matched) {
  let best = null, runStart = -1;
  for (let k = 0; k <= spans.length; k++) {
    const changed = k < spans.length && !matched[k];
    if (changed && runStart < 0) runStart = k;
    if (!changed && runStart >= 0) {
      const len = spans[k - 1].end - spans[runStart].start;
      if (!best || len > best.len) best = { start: spans[runStart].start, end: spans[k - 1].end, len };
      runStart = -1;
    }
  }
  return best;
...
function changedSpans(before, after) {
  ...
  const A = Aw.map(w => w.word), B = Bw.map(w => w.word);
  const n = A.length, m = B.length;
  const dp = Array.from({ length: n + 1 }, () => new Array(m + 1).fill(0));
  for (let i = n - 1; i >= 0; i--) for (let j = m - 1; j >= 0; j--) dp[i][j] = A[i] === B[j] ? dp[i + 1][j + 1] + 1 : Math.max(dp[i + 1][j], dp[i][j + 1]);
  const matchedA = new Array(n).fill(false), matchedB = new Array(m).fill(false);
  let i = 0, j = 0;
  while (i < n && j < m) {
    if (A[i] === B[j]) { matchedA[i] = true; matchedB[j] = true; i++; j++; }
    else if (dp[i + 1][j] >= dp[i][j + 1]) i++;
    else j++;
  }
  return { before: longestUnmatchedRun(Aw, matchedA), after: longestUnmatchedRun(Bw, matchedB) };
}
...
function splitExamplePreview(after, before) {
  ...
  if (total <= EXAMPLE_PREVIEW_MAX) {
    return { parts: [{ text, visible: true }], preview: text, hiddenLength: 0, totalLength: total, gated: false, anchor: 'whole' };
  }
  // 공개 창: 가장 많이 바뀐 자리를 가운데 두고 12~18자. 바뀐 자리를 못 찾으면 첫 구절.
  const changed = mostChangedSpan(String(before || ''), text);
  let start = 0, end = Math.min(total, EXAMPLE_PREVIEW_MAX), anchor = 'head';
  if (changed) {
    anchor = 'changed';
    const want = Math.max(EXAMPLE_PREVIEW_MIN, Math.min(EXAMPLE_PREVIEW_MAX, changed.len));
    start = Math.max(0, Math.min(changed.start, total - want));
    end = Math.min(total, start + want);
  }
  ...
  const beforeSpan = changedSpans(String(before || ''), text).before;
  return {
    parts,
    ...
    anchor,
    beforeFocus: beforeSpan ? { start: beforeSpan.start, end: beforeSpan.end } : null
  };
}
