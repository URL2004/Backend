...
function resolveRadarAxisPolicy({ profile, confidence, sentenceTotal } = {}) {
  ...
  const conditioned = (axis, spec) => {
    ...
    // 종류를 못 가리면 '참고'로 흐리게 보여주지 않는다 — 막대·등급이 있으면 수치처럼 읽히므로 아예 비활성.
    if (lowConfidence) return { status: 'off', metric: spec.metric || axis, target: null, reason: AXIS_LOW_CONFIDENCE_REASON };
