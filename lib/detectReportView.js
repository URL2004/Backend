...
function buildDetectReportView({
  ...
  const hasStatisticalSupport = !!require('./detectStatisticalAssist').sanitizeSupport(statisticalSupport);
  if (hasStatisticalSupport) {
    causeAnalysis.scoreBasis = 'model_and_style_statistics';
    causeAnalysis.label = '모델이 확인한 반복 근거와 학습된 문체 통계 신호를 함께 반영했어요. 아래 문장 근거만으로 전체 점수를 설명하는 것은 아니에요.';
  }
