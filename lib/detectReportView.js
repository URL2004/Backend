...
function resolveProfessorRadar(scoreValue) {
  ...
  if (score >= 50) {
    return {
      ...
      label: '높은 구간', headline: 'AI식 문체 점수 · 높은 구간',
      disclaimer: PROFESSOR_RADAR_DISCLAIMER
    };
  }
  if (score >= 21) {
    return {
      ...
      label: '중간 구간', headline: 'AI식 문체 점수 · 중간 구간',
      disclaimer: PROFESSOR_RADAR_DISCLAIMER
    };
  }
  return {
    status: 'ready', score, band: 'low', range: '0~20',
    label: '낮은 구간', headline: 'AI식 문체 점수 · 낮은 구간',
    disclaimer: PROFESSOR_RADAR_DISCLAIMER
  };
}
...
function resolveStyleBand(score, riskLevel) {
  ...
  // A cached/model risk label must not override the displayed numeric score.
...
function resolveContentEvidence(measurements = {}) {
  ...
  // 경험+수치를 함께 담은 한 문장은 근거 두 문장이 아니다. 신규 계측은
  // 문장별 합집합을 쓰고, 합집합이 없는 과거 페이로드는 기존 해석을 유지한다.
  const grounded = Math.min(total, details.reduce((sum, item) => sum +
    (Number.isFinite(item?.grounded) ? Math.max(0, Math.min(item.sents, item.grounded))
      : Math.max(0, Number(item?.lived) || 0) + Math.max(0, Number(item?.specific) || 0)), 0));
  ...
  if (total < CONTENT_MIN_SENTENCES) {
    return {
      status: 'limited', label: '문장이 적어 근거 확인 어려움', lived, specific, generic, total,
      grounded, groundedRatio: Number((grounded / total).toFixed(3))
    };
  }
  ...
  if (groundedRatio >= 0.35) {
    return {
      status: 'strong', label: '구체 근거 충분', lived, specific, grounded, generic, total,
      groundedRatio: Number(groundedRatio.toFixed(3))
    };
  }
  if (groundedRatio >= 0.125) {
    return {
      status: 'mixed', label: '구체 근거 일부', lived, specific, grounded, generic, total,
      groundedRatio: Number(groundedRatio.toFixed(3))
    };
  }
  return {
    status: 'weak', label: '근거 보강 필요', lived, specific, grounded, generic, total,
    groundedRatio: Number(groundedRatio.toFixed(3))
  };
}

// Raw counts are measurements, not a genre-independent instruction to add facts.
// Use the exact axis policy and metric used by the radar for every public claim.
function applyContentEvidencePolicy(evidence, axisPolicy) {
  const axis = axisPolicy.axes.anchor;
  const common = { ...evidence, policyVersion: axisPolicy.version, metric: axis.metric,
    target: axis.target ?? null, assessmentStatus: axis.status };
  if (evidence.status === 'limited') {
    return { ...common, reason: axis.reason, assessedRatio: null };
  }
  if (axis.status !== 'on') {
    return { ...common, status: 'not_assessed',
      label: axis.status === 'sparse' ? '내용 근거 측정 안 함' : '내용 근거 평가 대상 아님',
      reason: axis.reason, assessedRatio: null };
  }
  const numerator = axis.metric === 'lived' ? evidence.lived : evidence.grounded;
  const ratio = evidence.total > 0 ? numerator / evidence.total : 0;
  const status = ratio >= axis.target ? 'strong' : ratio >= axis.target / 2 ? 'mixed' : 'weak';
  const name = axis.metric === 'lived' ? '경험 문장' : '구체 근거';
  return { ...common, status, assessedRatio: Number(ratio.toFixed(3)),
    label: status === 'strong' ? `${name} 비율 기준 충족`
      : status === 'mixed' ? `${name} 비율 일부 확인` : `${name} 비율 낮음`, reason: null };
}

// ── 다듬을 대상(위치 검증된 문장) ─────────────────────────────────────────────
//   휴머나이징 유도는 점수 밴드가 아니라 "원문 위치에 연결된 문장과 이유"로 정한다(사장님 2026-09-07).
//   모델이 짚었지만 위치가 없는 신호, 문장 수, 통계 보조 적용 여부는 추천 근거가 아니다.
//   "후보 N문장"은 여기서 센 문장 수만 쓴다 — 지어낸 숫자가 아니다.
const MEASURED_CANDIDATE_CATEGORY = Object.freeze({ generic: 'generic_abstraction', endingRun: 'ending_repetition' });
function resolveConversionCandidates(signalEvidence, sentenceMap, { characters = null } = {}) {
  const { PATTERN_LABELS } = require('./detectInterpretation');
  const byCategory = new Map();
  const all = new Set();
  const add = (category, index) => {
    if (!Object.hasOwn(PATTERN_LABELS, category) || !Number.isSafeInteger(index) || index < 0) return;
    if (!byCategory.has(category)) byCategory.set(category, new Set());
    byCategory.get(category).add(index);
    all.add(index);
  };
  for (const item of Array.isArray(signalEvidence) ? signalEvidence : []) {
    if (!item || item.locationStatus !== 'source_range_verified') continue;
    for (const loc of Array.isArray(item.locations) ? item.locations : []) {
      if (![loc?.sentenceIndex, loc?.start, loc?.end].every(Number.isSafeInteger)) continue;
      if (loc.start < 0 || loc.end <= loc.start || (characters !== null && loc.end > characters)) continue;
      add(item.category, loc.sentenceIndex);
    }
  }
  // 결정론 계측이 문장 단위로 잡은 항목(일반 표현·같은 종결 반복)도 위치가 있는 근거다.
  const marks = sentenceMap && Array.isArray(sentenceMap.sentences) ? sentenceMap.sentences : [];
  for (const mark of marks) {
    if (!mark || !Number.isSafeInteger(mark.index)) continue;
    if (mark.kind === 'generic') add(MEASURED_CANDIDATE_CATEGORY.generic, mark.index);
    if (Number(mark.endingRun) >= ENDING_RUN_MIN) add(MEASURED_CANDIDATE_CATEGORY.endingRun, mark.index);
  }
  const reasons = [...byCategory.entries()]
    .map(([category, set]) => ({
      category, label: PATTERN_LABELS[category],
      sentenceIndexes: [...set].sort((a, b) => a - b).slice(0, 40), count: set.size
    }))
    .sort((a, b) => b.count - a.count || (a.category < b.category ? -1 : a.category > b.category ? 1 : 0));
  return { candidateSentences: all.size, reasons };
}

function buildSynthesis(styleBand, contentStatus, candidateSentences = 0, contentEvidence = {}) {
  const name = contentEvidence.metric === 'lived' ? '경험 문장' : '구체 근거';
  const evidenceClause = contentStatus === 'strong'
    ? `${name} 비율은 이 글 종류의 참고 기준을 충족해요.`
    : contentStatus === 'mixed'
      ? `${name} 비율은 참고 기준보다 낮게 측정됐어요.`
      : contentStatus === 'weak'
        ? `${name} 비율은 낮지만, 확인하지 않은 내용을 추가하지는 마세요.`
        : contentEvidence.reason || '내용 근거의 충분성은 평가하지 않았어요.';
  ...
  if (styleBand === 'low') {
    // 낮은 점수라도 위치가 확인된 문장이 있으면 그 문장만 다듬을 수 있다고 말한다.
    // "유지해도 좋아요"는 지목할 문장이 하나도 없을 때만 — 개선점 없는 글에 수정을 권하지 않는다.
    return {
      headline: `AI식 문체 점수는 낮은 구간이고, ${evidenceClause}`,
      description: candidateSentences > 0
        ? `점수는 낮지만 ${candidateSentences}문장에서 정형 패턴이 확인돼요. 그 문장만 골라 다듬을 수 있어요.`
        : contentStatus === 'weak'
...
function buildDetectReportView({
  ...
  statisticalSupport = null,
  statisticalReference = null,
  inputIncomplete = false,
  eligibleSentenceCount = null,
  measurements = {},
  ...
  const styleBandLabel = { low: '낮은 구간', moderate: '중간 구간', high: '높은 구간', unknown: '점수 확인 필요' }[styleBand];
  const rawContentEvidence = resolveContentEvidence(measurements);
  const measurable = score !== null && rawContentEvidence.status !== 'limited' && !sourceLimited;
  ...
  const axisPolicy = resolveRadarAxisPolicy({
    ...
    sentenceTotal: rawContentEvidence.total
  });
  const contentEvidence = applyContentEvidencePolicy(rawContentEvidence, axisPolicy);
  const support = require('./detectStatisticalAssist').sanitizeSupport(statisticalSupport);
  const supportScore = calibrationApplied === true ? normalizeScore(preCalibrationProbability) : score;
  const verifiedStatisticalSupport = support?.score === supportScore ? support : null;
  const causeAnalysis = assessCauseCoverage(
    ...
    Array.isArray(signalEvidence) && signalEvidence.length ? signalEvidence : signals,
    {
      ...
      statisticalSupport: verifiedStatisticalSupport
    }
  );
  ...
  const hasStatisticalSupport = !!verifiedStatisticalSupport;
  if (hasStatisticalSupport) {
    const independent = verifiedStatisticalSupport.basis === 'independent_statistics';
    causeAnalysis.preCalibrationBasis = independent ? 'style_statistics' : 'model_and_style_statistics';
    if (calibrationApplied !== true) causeAnalysis.scoreBasis = causeAnalysis.preCalibrationBasis;
    causeAnalysis.label = independent
      ? '학습된 문체 통계 신호를 점수에 함께 반영했어요. 문장별 원인 설명은 충분하지 않을 수 있어요.'
      : '모델이 확인한 반복 근거와 학습된 문체 통계 신호를 함께 반영했어요. 아래 문장 근거만으로 전체 점수를 설명하는 것은 아니에요.';
  }
  const interpretation = buildDetectInterpretation({
    probability: score, probSource: source, confidence, textLength,
    sentenceTotal: Number.isSafeInteger(eligibleSentenceCount) ? eligibleSentenceCount : contentEvidence.total,
    signalEvidence,
    statisticalSupport: verifiedStatisticalSupport,
    statisticalReference: require('./detectStatisticalAssist').sanitizeReference(statisticalReference),
    calibrationApplied,
    preCalibrationProbability,
    inputIncomplete,
    causeCoverageStatus: causeAnalysis.status
  });
  // Numeric support coverage is not the same as a located explanation. A
  // statistics-only lift or weak/unlocated causes cannot advertise full alignment.
  const explanationPartial = hasStatisticalSupport || interpretation.status === 'partial';
  causeAnalysis.explanationStatus = explanationPartial ? 'partial' : interpretation.status;
  if (explanationPartial && !hasStatisticalSupport) {
    causeAnalysis.label = '점수는 표시하되 원문 위치에 연결된 원인 설명은 일부만 확인됐어요.';
  }
  const reportStatus = !measurable || ['limited', 'unavailable'].includes(interpretation.status)
    ? 'limited'
    : causeAnalysis.status === 'aligned' && !explanationPartial
      ? 'ready'
      : 'partial';
  const candidates = resolveConversionCandidates(signalEvidence, sentenceMap, { characters: normalizeCount(textLength) });
  const synthesis = sourceLimited
    ...
    : ['limited', 'unavailable'].includes(interpretation.status)
      ? { headline: interpretation.headline, description: interpretation.description, limitation: SYNTHESIS_LIMITATION }
    : explanationPartial || causeAnalysis.status === 'partial'
      ? {
          headline: hasStatisticalSupport ? '문체 통계가 반영된 점수와 문장별 근거를 구분해 주세요.' : '점수는 확인됐지만 원인 설명은 일부만 연결됐어요.',
          description: candidates.candidateSentences > 0
            ? `표시된 원인만으로 점수를 충분히 설명하기는 어려워요. 위치가 확인된 ${candidates.candidateSentences}문장부터 다듬을 수 있어요.`
            : '표시된 원인만으로 점수를 충분히 설명하기 어려워 단정적인 수정 권고는 하지 않아요.',
          limitation: SYNTHESIS_LIMITATION
        }
      : buildSynthesis(styleBand, contentEvidence.status, candidates.candidateSentences, contentEvidence);
  // 접근(access)은 점수 밴드와 무관하다 — 모델 판정이 완료된 글이면 휴머나이징으로 갈 수 있다.
  // 추천(recommend)은 위치가 확인된 문장이 있을 때만. 둘 다 아니면 입력 확인을 권한다.
  const conversionAccess = measurable;
  const conversionRecommend = conversionAccess && candidates.candidateSentences >= 1;
  ...
  return {
    ...
    styleSignal: {
      ...
      score,
      scoreKind: 'ai_style',
      scoreLabel: 'AI식 문체 점수',
      band: styleBand,
      bandLabel: styleBandLabel,
      label: `AI식 문체 점수 · ${styleBandLabel}`,
      source,
      sourceLabel: source === 'llm' ? (hasStatisticalSupport ? 'AI 모델·문체 통계 결합 분석' : 'AI 모델 분석') : '문체 엔진 간이 추정',
      calibrated: calibrationApplied === true
    },
    professorRadar,
    contentEvidence,
    ...
    measuredEvidence: {
      ...
      groundedCount: contentEvidence.grounded ?? Math.min(contentEvidence.total, contentEvidence.lived + contentEvidence.specific),
    ...
    conversion: {
      version: 'conversion-v2-located',
      access: conversionAccess,
      recommend: conversionRecommend,
      // 구버전 화면 호환 — access와 같은 값. 점수 밴드로 유도를 끄던 규칙은 폐기했다.
      eligible: conversionAccess,
      candidateSentences: candidates.candidateSentences,
      reasons: candidates.reasons,
      action: conversionRecommend ? 'review_recommendation' : conversionAccess ? 'optional_review' : 'keep_or_review_input'
    }
  };
}
...
function buildSentenceMap(paras, detail, { sourceParagraphs = paras, source = null } = {}) {
  const paragraphs = Array.isArray(paras) ? paras : [];
  const marks = [];
  const paragraphRows = [];
  const sourceOffsets = typeof source === 'string'
    ? require('./detectReportLocations').reportSourceOffsets(source, sourceParagraphs) : null;

  paragraphs.forEach((paragraph, paragraphIndex) => {
    const spans = splitSentenceSpans(paragraph);
    const sentences = spans.map(span => span.text);
    const row = {
      index: paragraphIndex,
      kind: detail?.[paragraphIndex]?.kind || 'thin',
      head: String(sourceParagraphs[paragraphIndex] || paragraph).replace(/\s+/g, ' ').trim().slice(0, 40),
      ...(detail?.[paragraphIndex]?.excluded ? { excluded: true } : {}),
    ...
    sentences.forEach((sentence, sentenceIndex) => {
      ...
      const displayed = String(sourceParagraphs[paragraphIndex] || paragraph).slice(span.start, span.end);
      ...
      marks.push({
        ...
        ...(sourceOffsets ? { start: sourceOffsets[paragraphIndex][span.start],
          end: sourceOffsets[paragraphIndex][span.end - 1] + 1 } : {}),
        length: sentence.replace(/\s+/gu, ' ').length,
        kind,
        ending: sentenceEnding(sentence),
        endingRun: 0,
        runStart: false,
        ...(displayed !== sentence ? { protectedContentExcluded: true } : {}),
        text: displayed.length > SENTENCE_TEXT_CAP
          ? `${displayed.slice(0, SENTENCE_TEXT_CAP)}…`
          : displayed
      });
    });
  ...
  // Full, text-free coordinates remain available even when UI excerpts are capped.
  const sourceLocations = sourceOffsets ? marks.map(({ index, paragraph, start, end }) => ({ index, paragraph, start, end })) : [];
  // This full index is request-local. Serializing it would defeat the excerpt
  // cap and bloat paid-response/history documents for thousands of sentences.
  const withLocations = result => Object.defineProperty(result, 'sourceLocations', { value: sourceLocations });
  ...
  if (total <= SENTENCE_SHOW_CAP) {
    countExcerpts(marks);
    return withLocations({ total, shown: total, capped: false, maxEndingRun, sentences: marks, paragraphs: paragraphRows });
  }
  ...
  return withLocations({ total, shown: sentences.length, capped: true, maxEndingRun, sentences, paragraphs: paragraphRows });
}
