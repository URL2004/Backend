...
async function writeResultState(input, response, status, now) {
  ...
  if (status === 'RESULT_READY' && ['RESULT_READY', 'COMPLETE'].includes(localExisting?.status) && localExisting.response) {
    return { ...stateFromRow(localExisting), reused: true };
  }
  memory.set(key, {
    purpose: PURPOSE,
    ...binding,
  ...
  if (!db || !admin) return { state: status, response: safeResponse, reused: false };
  ...
  try {
    return await db.runTransaction(async transaction => {
      ...
      if (status === 'RESULT_READY' && ['RESULT_READY', 'COMPLETE'].includes(existing.status) && existing.response) {
        return { ...stateFromRow(existing), reused: true };
      }
      ...
      return { state: status, response: safeResponse, reused: false };
    });
  } catch (error) {
