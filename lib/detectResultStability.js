...
const VERSION = 'detect-result-stability-v4-sample-units';
...
// Raw detector cache identity is independent of the UI route and billing
// request. Preserve exact text so cached evidence offsets remain valid.
function payloadFingerprint({ text, lang = 'ko', referenceContext = '' } = {}) {
  return crypto.createHash('sha256').update(JSON.stringify({
    version: 'detect-raw-input-v1',
    text: String(text || ''),
    lang: lang === 'en' ? 'en' : 'ko',
    referenceContext: String(referenceContext || '').trim().slice(-300)
  }), 'utf8').digest('hex');
}
...
function cleanResult(value) {
  ...
  return {
    ...
    ...(require('./detectStatisticalAssist').sanitizeReference(value.statisticalReference) ? {
      statisticalReference: require('./detectStatisticalAssist').sanitizeReference(value.statisticalReference)
    } : {}),
    ...(require('./detectStatisticalAssist').sanitizeSupport(value.statisticalSupport) ? {
      statisticalSupport: require('./detectStatisticalAssist').sanitizeSupport(value.statisticalSupport)
    } : {}),
    ...(require('./detectDiagnostics').sanitizeDiagnostics(value.detectDiagnostics) ? {
      detectDiagnostics: require('./detectDiagnostics').sanitizeDiagnostics(value.detectDiagnostics)
    } : {}),
...
function cleanLocations(item) {
  ...
  const locations = (Array.isArray(item.locations) ? item.locations : [])
    ...
    .slice(0, 8)
    .map(({ sentenceIndex, start, end, sampleUnitIndex, paragraphIndex }) => ({ sentenceIndex, start, end,
      ...(Number.isSafeInteger(sampleUnitIndex) && sampleUnitIndex >= 0 ? { sampleUnitIndex } : {}),
      ...(Number.isSafeInteger(paragraphIndex) && paragraphIndex >= 0 ? { paragraphIndex } : {}) }));
...
function variantForConfig(config = {}, options = {}) {
  const payload = {
    ...
    inputDocumentVersion: require('./detectInputDocument').DETECT_INPUT_DOCUMENT_VERSION,
    statisticalAssistVersion: require('./detectStatisticalAssist').VERSION,
    statisticalModelVersion: require('./detectStatisticalAssist').MODEL_VERSION,
    statisticalAssistEnabled: require('./detectStatisticalAssist').enabled(),
    independentStatisticsEnabled: require('./detectStatisticalAssist').independentEnabled(),
    assignmentClassifierVersion: require('./detectAssignmentClassifier').VERSION,
    assignmentClassifierModelVersion: require('./detectAssignmentClassifier').MODEL_VERSION,
    assignmentClassifierEnabled: require('./detectAssignmentClassifier').enabled(),
    assignmentClassifierThreshold: require('./detectAssignmentClassifier').enabled() ? require('./detectAssignmentClassifier').highThreshold() : null,
