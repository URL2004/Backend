...
function calibratePrediction(raw, model, policy) {
  if (!raw) return null;
...
function predict(text, model, policy) {
  ...
  return calibratePrediction(require('./detectStyleClassifier').predict(text, model), model, policy);
}

// Production shadow uses a private immutable snapshot. Validate and hash it
// once, then derive both research scores from one classifier evaluation.
// The regular predict API still validates each caller-supplied mutable policy.
function createPredictor(model, policy) {
  const classifier = require('./detectStyleClassifier');
  const snapshot = structuredClone({ model, policy });
  if (!classifier.validateModel(snapshot.model) || !validatePolicy(snapshot.policy, snapshot.model)) {
    throw Error('risk_invalid_policy');
  }
  const freeze = value => {
  ...
  freeze(snapshot);
  return Object.freeze({ modelDigest: snapshot.policy.modelDigest, policyDigest: snapshot.policy.digest,
    predict(text) {
      const raw = classifier.predict(text, snapshot.model);
      return { raw, calibrated: calibratePrediction(raw, snapshot.model, snapshot.policy) };
    }
  });
}
...
module.exports = { VERSION, selectRank, fitCalibration, validatePolicy, predict, createPredictor, assertIndependentValidation };
