...
function alignScoreToCauseEvidence(result = {}) {
  const probability = finiteScore(result.probability);
...
function assessCauseCoverage(scoreValue, evidenceValue, {
  source = 'llm',
  calibrated = false,
  statisticalSupport = null
} = {}) {
  const score = finiteScore(scoreValue);
  const evidence = normalizeSignalEvidence(evidenceValue);
  ...
  const hasStatisticalSupport = statisticalSupport?.applied === true;
  const statisticalIndependentSignals = hasStatisticalSupport ? 1 : 0;
  const independentSignalCount = qualifying.length + statisticalIndependentSignals;
  const codes = [];
  ...
  if (!Number.isFinite(score) || source !== 'llm') {
    ...
    if (independentSignalCount < requirement.independent) codes.push('cause_count_below_score_band');
  ...
  const covered = Math.min(required, independentSignalCount);
  return {
    ...
    // Legacy numeric field is policy coverage, not an evidence-confidence rate.
    coverageMeaning: required === 0 ? 'not_required_at_this_score' : 'required_signal_coverage',
    requiredIndependentSignals: required,
    qualifyingIndependentSignals: qualifying.length,
    statisticalIndependentSignals,
    codes,
