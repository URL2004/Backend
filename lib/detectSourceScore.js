...
function inputHash(text) {
  return crypto.createHash('sha256').update(exactText(text), 'utf8').digest('hex');
}
...
async function resolveSourceScore({ db, uid, text, claimedScore }) {
  ...
  if (!db || !uid || !exactText(text)) return null;
  const history = db.collection('users').doc(uid).collection('history');
  const fields = ['type', 'savedBy', 'probSource', 'inputText', 'probability', 'probabilityCalibration.applied'];
  // The hash index finds the latest source even after many unrelated runs.
  // Keep the bounded legacy scan until older histories have been indexed.
  let indexed = [];
  if (typeof history.where === 'function') {
    const snapshot = await history.where('detectInputHash', '==', inputHash(text))
      .orderBy('createdAt', 'desc').limit(1).select(...fields).get();
    indexed = snapshot.docs;
  }
  const query = history
    .orderBy('createdAt', 'desc').limit(LOOKUP_LIMIT)
    .select(...fields);
  const authoritative = record => record.type === 'detect' && record.savedBy === 'server'
    && ['llm', 'cached_llm'].includes(record.probSource) && exactText(record.inputText) === exactText(text);
  const docs = indexed.some(doc => authoritative(doc.data() || {})) ? indexed : (await query.get()).docs;
  for (const doc of docs) {
    const record = doc.data() || {};
    if (record.type !== 'detect' || record.savedBy !== 'server' || !['llm', 'cached_llm'].includes(record.probSource)) continue;
    ...
    return score === null || measured === score ? measured : null;
  }
  return null;
}
...
module.exports = { VERSION, LOOKUP_LIMIT, optionalScore, inputHash, resolveSourceScore, signSourceScore, verifiedSourceScore };
