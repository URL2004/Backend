...
const VERSION = 'statistical-assist-v2';
...
function sanitizeSupport(value) {
  ...
  if (!value || ![VERSION, 'statistical-assist-v1'].includes(value.version) || value.applied !== true
    || ![value.originalScore, value.score, value.margin].every(Number.isFinite)
    || value.originalScore < (independent ? 0 : 21) || value.originalScore >= 50 || value.score < 50 || value.score > 74
    || value.margin <= 0 || !Number.isSafeInteger(value.features) || value.features < 100 || value.features > 30000
    || (independent && (value.version !== VERSION || value.margin < 0.05))
...
function applyAssist(result, source, { profile, active = enabled(), independentActive = independentEnabled(), modelValue } = {}) {
  ...
  if (!active || !PROFILES.has(profile) || typeof source !== 'string'
    ...
    || typeof originalScore !== 'number' || !Number.isFinite(originalScore) || originalScore < 0 || originalScore >= 50) return result;
  ...
  const independentScope = independentActive && result.confidence !== 'low'
    && require('./detectGrounding').sourceSentences(source).length >= 4
    && !/(?:^|\n)\s*(?:>|\||```|참고문헌|references\b)/imu.test(source)
    && !/[“"][^”"]{100,}[”"]/u.test(source);
