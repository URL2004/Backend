...
function sanitizeSupport(value) {
  // 과제 도메인 독립 분류기 지원 메타는 자기 계약으로 정화한다(순환 참조 방지를 위해 지연 로드).
  if (value?.version === 'assignment-classifier-v1') return require('./detectAssignmentClassifier').sanitizeAssignmentSupport(value);
