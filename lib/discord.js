...
function send(channel, embed, extra) {
  ...
  return postWebhook(url, payload).then((ok) => {
    if (ok) { sendStats.sent++; return true; }
    sendStats.failed++;
  ...
  opsAlert(record, classification, meta) {
    ...
    send(channel, {
      ...
      fields: [
        ...
        // 결제 실패 분류(2026-09-04) — "돈이 나갔는가"를 알림 본문에서 바로 읽게 한다.
        record.outcome ? field('분류', `${record.outcome}${record.failureCategory ? ` · ${record.failureCategory}` : ''}`) : null,
        record.moneyAtRisk != null ? field('돈 위험', record.moneyAtRisk ? '있음 — 승인 여부 확인 필요' : '없음(미결제)') : null,
        record.providerStatus ? field('결제사 상태', record.providerStatus) : null,
        record.providerMessage ? field('결제사 원문', record.providerMessage) : null,
        // 한 사람의 반복인지 여러 사람의 장애인지 — 이 줄 하나로 구분된다.
        record.retryUidFailures5m > 1
          ? field('재시도', `이 회원 5분 내 ${record.retryUidFailures5m}회 · 주문 ${record.retryUidDistinctOrders5m || 1}건${record.repeatedDecline ? ' · 같은 사유 반복' : ''}`)
          : null,
        record.surgeSubjects ? field('영향 범위', `서로 다른 ${record.surgeSubjects}건(회원·주문) · 총 ${record.occurrences || record.surgeSubjects}건`) : null,
