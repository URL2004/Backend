...
const DEFAULT_CONFIG = {
  ...
  escalation: {
    enabled: true,
    longTextChars: 9000,
    protectedTermThreshold: 35,
    patchTargetThreshold: 24
  }
};
...
function provider(value, fallback, allowNone = false) {
  if (value == null) return fallback;
  const v = String(value || '').trim().toLowerCase();
  if (allowNone && (v === 'none' || v === 'off')) return 'none';
...
function envConfig() {
  ...
  return sanitizeConfig({
    activeProvider: process.env.LLM_ACTIVE_PROVIDER || DEFAULT_CONFIG.activeProvider,
    models: {
