...
const VERSION = 'gpt-runtime-config-v2';
...
function migrateLegacyModel(value) {
  ...
  if (/^gpt-5\.4-(?:mini|nano)(?:-\d{4}-\d{2}-\d{2})?$/.test(lower)) return 'gpt-5.6-luna';
  if (/^gpt-5\.4(?:-\d{4}-\d{2}-\d{2})?$/.test(lower)) return 'gpt-5.6-terra';
  return modelId;
}
...
function migrateStoredConfig(raw = {}) {
  ...
  const hasLegacyModels = Object.values(storedModels).some(value => {
    ...
    return current && migrateLegacyModel(current) !== current;
  });
