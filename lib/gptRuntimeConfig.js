...
const VERSION = 'gpt-runtime-config-v3';
...
const DEFAULT_CONFIG = {
  ...
  reasoning: {
    // Bounded classification/detection: low. Generation, local repair and
    // primary semantic review: medium. Dense facts and escalation: high.
    // Structure planning uses judgeEscalation + judge effort (Sol medium).
    humanize: 'medium',
    factDense: 'high',
    escalation: 'high',
    judge: 'medium',
...
const LEGACY_REASONING_DEFAULTS = Object.freeze({
  humanize: 'low',
  factDense: 'medium',
  escalation: 'medium',
  judge: 'medium',
...
function model(value, fallback) {
  ...
  const clean = (v || String(fallback || '').trim()).replace(/[^\w.:/-]/g, '').slice(0, 80);
  return migrateLegacyModel(clean) || fallback;
}

function migrateLegacyModel(value) {
  const modelId = String(value || '').trim();
  const lower = modelId.toLowerCase();
  if (/^gpt-(?:5\.4-(?:mini|nano)|5\.6-luna)(?:-\d{4}-\d{2}-\d{2})?$/.test(lower)) return 'gpt-6-luna';
  if (/^gpt-(?:5\.4|5\.6-terra)(?:-\d{4}-\d{2}-\d{2})?$/.test(lower)) return 'gpt-6-sol';
  return modelId;
}
...
function reasoning(value, fallback) {
  ...
  return ['none', 'low', 'medium', 'high', 'xhigh', 'max', 'default', 'minimal'].includes(v) ? v : fallback;
}
...
function migrateStoredConfig(raw = {}) {
  const stored = raw && typeof raw === 'object' ? raw : {};
  const storedModels = stored.models && typeof stored.models === 'object' ? stored.models : {};
  const hasLegacyModels = Object.values(storedModels).some(value => {
    const current = String(value || '').trim();
    // Only v1/5.4 profiles need the old effort migration. Model migration
    // from 5.6 must not silently overwrite an administrator's tuned efforts.
    return /^gpt-5\.4(?:-|$)/i.test(current);
  });
  const isLegacyProfile = String(stored.version || '') === 'gpt-runtime-config-v1' || hasLegacyModels;
  if (!isLegacyProfile) return stored;

  const storedReasoning = stored.reasoning && typeof stored.reasoning === 'object'
    ? stored.reasoning
    : {};
  const migratedReasoning = { ...storedReasoning };
  for (const [key, oldDefault] of Object.entries(LEGACY_REASONING_DEFAULTS)) {
    if (String(storedReasoning[key] || '').trim().toLowerCase() === oldDefault) {
      migratedReasoning[key] = DEFAULT_CONFIG.reasoning[key];
    }
  }
  return {
    ...stored,
    models: { ...storedModels },
    reasoning: migratedReasoning
  };
}
...
async function getRuntimeConfig({ db, logger, force = false } = {}) {
  ...
  try {
    ...
    const stored = snap.exists ? migrateStoredConfig(snap.data() || {}) : {};
    const merged = {
      ...envCfg,
      ...stored,
