...
async function saveAnalyzeHistory({
  ...
  if (isDetect) {
    ...
    // 보고서 → 휴머나이징 핸드오프로 들어온 작업은 원글의 감지 점수와 근거 수를 함께 남긴다.
    //   재검사 보정의 원점수 상한(detectCalibration)과 결과 화면의 "유지할 근거 보존" 표기에 쓴다.
    if (Number.isFinite(Number(sourceProbability))) {
      doc.sourceProbability = Math.max(0, Math.min(100, Math.round(Number(sourceProbability))));
    }
