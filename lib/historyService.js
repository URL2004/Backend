...
const HISTORY_PROVENANCE_VERSION = 1;

function historyProvenanceSecret() {
  return String(process.env.HISTORY_PROVENANCE_SECRET || process.env.OPENAI_SAFETY_SALT || '').trim();
}

function historyProvenancePayload({ uid, type, mode, outputText }) {
  const outputHash = crypto.createHash('sha256').update(String(outputText || ''), 'utf8').digest('hex');
  return ['gp-history', HISTORY_PROVENANCE_VERSION, String(uid || ''), String(type || ''), String(mode || ''), outputHash].join('\0');
}

function buildHistoryProvenance(fields, secret = historyProvenanceSecret()) {
  if (!secret || !fields?.uid || !fields?.type) return null;
  return {
    version: HISTORY_PROVENANCE_VERSION,
    signature: crypto.createHmac('sha256', secret)
      .update(historyProvenancePayload(fields), 'utf8')
      .digest('hex')
  };
}

function verifyHistoryProvenance(fields, provenance, secret = historyProvenanceSecret()) {
  if (provenance?.version !== HISTORY_PROVENANCE_VERSION || !/^[a-f0-9]{64}$/u.test(String(provenance?.signature || ''))) {
    return false;
  }
  const expected = buildHistoryProvenance(fields, secret)?.signature;
  if (!expected) return false;
  const actualBuffer = Buffer.from(provenance.signature, 'hex');
...
async function saveAnalyzeHistory({
  ...
  const provenance = buildHistoryProvenance({
    uid,
    type: doc.type,
    mode: doc.mode,
    outputText: doc.outputText || ''
  });
  if (provenance) doc.serverProvenance = provenance;
...
module.exports = {
  HISTORY_PROVENANCE_VERSION,
  buildHistoryProvenance,
  verifyHistoryProvenance,
  CURRENT_BILLING_DISPOSITIONS,
