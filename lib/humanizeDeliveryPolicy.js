...
const EFFECT_ONLY_GATES = new Set([
  ...
  // 목표 문단 수 또는 권장 길이를 모두 맞추지 못했다는 뜻이며,
  // 잠긴 제목·라벨 손실이나 문단 역할 이동은 별도 구조 게이트가 담당한다.
  ...
  // 현행 v2 방출처는 0이지만 과거 작업/관리자 shadow 자료를 읽을 때
  // 검토 필요로 승격하지 않도록 읽기 호환 엔트리로만 유지한다.
...
function buildAllowedExtra({ evidence = '', userNotes = '' } = {}) {
  const parts = [];
  const context = partitionRequestContext({ evidence, userNotes });
  const evidenceText = context.evidenceFacts.join('\n');
  const noteText = context.authorFacts.join('\n');
  if (evidenceText) parts.push(`[사용자 제공 근거]\n${evidenceText}`);
  if (noteText) parts.push(`[사용자 메모]\n${noteText}`);
  return parts.join('\n\n');
}

function reconcileFinalDelivery({
  blocked = false,
  baseReasonCodes = [],
  qualityWarnings = []
} = {}) {
  const warningCodes = (qualityWarnings || []).map(gateOf).filter(Boolean);
  const reasonCodes = [...new Set((blocked
    ? baseReasonCodes
    : [...baseReasonCodes, ...warningCodes])
