...
function restructureStructureCredit(length) {
  return Math.ceil(restructureBaseCredit(length) * 0.30);
}

function restructureCredit(length, evidenceEnabled = false, structureEnabled = false) {
  const base = restructureBaseCredit(length);
  return base + (evidenceEnabled ? restructureEvidenceCredit(length) : 0)
    + (structureEnabled ? restructureStructureCredit(length) : 0);
}
