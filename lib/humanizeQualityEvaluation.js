...
// A fixed 2x2 design isolates prompt compression from change-amount pressure.
// The fifth arm adds the opt-in relation guard on the compact base (the priority
// candidate after the 2026-09-08 live ablation); the four 2x2 environments are
// unchanged, so control and its stored outputs remain the reference.
// These are explicit evaluation settings, not a recommendation to activate a
// production flag. Existing delivery/no-op and calibration policies still run.
function humanizeAblationMatrix() {
  return [
    ['control', 'full', 'perceived', []],
    ['compact_only', 'compact_v1', 'perceived', ['prompt_compression']],
    ['issue_focus_only', 'full', 'issue_focused_v1', ['edit_objective']],
    ['compact_issue_focus', 'compact_v1', 'issue_focused_v1', ['prompt_compression', 'edit_objective']],
    ['relation_guard', 'compact_v1', 'perceived', ['prompt_compression', 'relation_guard'], 'clear_relations_v1']
  ].map(([id, promptVariant, editObjective, changedFactors, relationGuard]) => ({
    id,
    version: 'humanize-ablation-v1',
    environment: { HUMANIZE_PROMPT_VARIANT: promptVariant, HUMANIZE_EDIT_OBJECTIVE: editObjective,
      ...(relationGuard ? { HUMANIZE_RELATION_GUARD: relationGuard } : {}) },
    changedFactors,
    releaseEligible: false,
    primaryOutcomes: ['content_preservation', 'blind_naturalness_with_reversed_order', 'overblocking'],
    secondaryOutcomes: ['source_recalibrated_detector_delta', 'latency', 'cost'],
    unchangedPolicies: ['delivery_and_noop', 'source_score_calibration'],
    comparisonRequirements: ['same_source_family_split', 'same_model_snapshot', 'same_genre_strength', 'prompt_and_output_digests', 'human_adjudication']
  }));
}
module.exports = { DIMENSIONS, FIDELITY, makeBlindPair, qualityPrompt, fidelityPrompt, qualitySchema, fidelitySchema, verifyQuality, verifyFidelity, numberAudit, summarizeQuality, humanizeAblationMatrix };
