...
const METRIC_FIELDS = Object.freeze([
  ...
  'macroDiscourseScore',
  'macroDiscourseSourceParagraphCount',
  'macroDiscourseOutputParagraphCount',
  'macroDiscourseRecomposedParagraphCount',
  'macroDiscourseRepeatedEvaluationReduction',
  'macroDiscourseRoleOrderRetention',
  ...
  'quoteDuplicateReductionCount',
  'quoteMissingUniqueCount',
...
function publicRecentRow(row) {
  const fields = [
    ...
    'modelFailureChunkCount', 'textualRefusalAttemptCount', 'textualRefusalChunkCount',
    'textualRefusalRecoveredChunkCount', 'textualRefusalUnrecoveredChunkCount',
    'chunkConcurrency', 'structureSignaturePass', 'sectionPathErrorCount',
    'humanizationDepthApplicable', 'humanizationDepthPass', 'humanizationOverallDepthPass',
    'humanizationTargetDepthMet', 'humanizationEditTargetMet',
    ...
    'macroDiscourseApplicable', 'macroDiscourseScore', 'macroDiscoursePass', 'macroDiscourseOrderPass',
    'macroDiscourseSourceParagraphCount', 'macroDiscourseOutputParagraphCount',
    'macroDiscourseRecomposedParagraphCount', 'macroDiscourseRepeatedEvaluationReduction',
    'macroDiscourseRoleOrderRetention', 'macroDiscourseIdeaOrderRetention',
    ...
    'unsupportedSpecificityPass', 'unsupportedSpecificityIssueCount',
    'unsupportedSpecificityResidualCount', 'unsupportedSpecificityRestoreCount',
    ...
    'quoteIntegrityPass', 'quoteCountChanged', 'quoteDuplicateReductionBenign',
    'quoteDuplicateReductionCount', 'quoteMissingUniqueCount', 'quoteContentChangedCount',
...
function buildHumanizeQualityReport(rows, options = {}) {
  ...
  return {
    ...
    summary: {
      ...
      unsupportedSpecificityIssueDocumentCount: completedRows.filter(
        row => Number(row.unsupportedSpecificityIssueCount) > 0
      ).length,
      unsupportedSpecificityResidualDocumentCount: completedRows.filter(
        row => Number(row.unsupportedSpecificityResidualCount) > 0
      ).length,
      unsupportedSpecificityRepairedDocumentCount: completedRows.filter(row => (
        Number(row.unsupportedSpecificityRestoreCount) > 0
          || Number(row.unsupportedSpecificityRemovalCount) > 0
      )).length,
