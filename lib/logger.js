...
const SENSITIVE_KEY_RE = /(^|_|-|\b)(authorization|cookie|password|secret|idtoken|id_token|access_token|refresh_token|api[_-]?key|paymentkey|billingkey|authkey|customerkey|cardnumber|signature|cvv|cvc|email|phone)(_|-|$|\b)/i;
...
const SENSITIVE_NORMALIZED_SUFFIXES = [
  'password',
  'passwd',
  'secret',
  'privatekey',
...
const EMBEDDED_EMAIL_RE = /([A-Z0-9._%+-]{1,64})@([A-Z0-9.-]+\.[A-Z]{2,})/gi;
const BEARER_RE = /\bBearer\s+[A-Z0-9._~+/=-]+/gi;
const JWT_RE = /\b[A-Z0-9_-]{8,}\.[A-Z0-9_-]{8,}\.[A-Z0-9_-]{8,}\b/gi;
const SECRET_QUERY_RE = /(^|[?&\s])((?:token|id_token|access_token|refresh_token|apiKey|paymentKey|signature)=)([^&#\s]*)/gim;

function isSensitiveKey(key) {
  const text = String(key || '');
  if (!text) return false;
  if (SENSITIVE_KEY_RE.test(text)) return true;
  const normalized = text.replace(/[^a-z0-9]/gi, '').toLowerCase();
  return SENSITIVE_NORMALIZED_KEYS.has(normalized)
    || SENSITIVE_NORMALIZED_SUFFIXES.some((suffix) => normalized.endsWith(suffix));
}
...
function maskEmailParts(head, domain) {
...
function sanitizeString(value, max = MAX_STRING) {
  ...
  text = text.replace(SECRET_QUERY_RE, (_, prefix, key) => `${prefix}${key}[REDACTED]`);
  text = text.replace(BEARER_RE, 'Bearer [REDACTED]');
  text = text.replace(JWT_RE, '[REDACTED_JWT]');
  text = text.replace(EMBEDDED_EMAIL_RE, (_, head, domain) => maskEmailParts(head, domain));
  return text.length > max ? `${text.slice(0, max)}...[truncated ${text.length - max}]` : text;
}

function serializeError(err) {
  if (!err) return undefined;
  if (typeof err === 'string') return { message: sanitizeString(err) };
  return {
    name: err.name || 'Error',
    message: sanitizeString(err.message || String(err)),
    code: err.code,
    status: err.status || err.statusCode,
    stack: process.env.LOG_STACKS === '1' && err.stack ? sanitizeString(err.stack) : undefined
  };
}
...
function baseRecord(level, event, fields) {
  ...
  // 가입 임계값은 여러 계정이 한 추세로 병합되는 집계 사건이다. 개별 UID를 어느 로그 싱크에도 싣지 않는다.
  if (event === 'account.initialize_quota_exceeded') {
    record.uid = undefined;
    record.actorUid = undefined;
  }
  // Research telemetry has its own closed projection. Request-scoped identity
  // must not be silently reintroduced by AsyncLocalStorage or a future caller.
  if (event === 'detect_report.evidence_shadow') {
    for (const key of Object.keys(ctx)) {
      if (key !== 'requestId' && !Object.hasOwn(fields || {}, key)) record[key] = undefined;
    }
    for (const key of ['uid', 'actorUid', 'userId', 'text', 'sourceText', 'inputText', 'outputText']) record[key] = undefined;
  }
...
function notifyOps(record, level) {
  try {
    ...
    if (outcome.surge) {
      // 임계 판정은 "서로 다른 주체 수"로 한다(회원·주문 단위). 한 사람의 재시도 반복이
      // 전체 장애로 승격되던 문제를 막는다 — 건수는 규모 파악용으로 함께 싣는다.
      log('error', 'ops.rate_threshold_exceeded', {
        surgeDomain: outcome.surge.domain,
        surgeSubjects: outcome.surge.subjects,
        ...
        message: `${outcome.surge.domain} 도메인에서 ${outcome.surge.windowMinutes}분간 서로 다른 ${outcome.surge.subjects}건(회원·주문 기준)에서 실패 ${outcome.surge.count}건 발생(임계 ${outcome.surge.threshold})`
      });
    }
