...
const { outboundFetch } = require('./outboundPolicy');
...
async function send(event) {
  ...
  try {
    const response = await outboundFetch('meta',
      `https://graph.facebook.com/${encodeURIComponent(graphVersion)}/${encodeURIComponent(datasetId)}/events`,
      {
        method: 'POST',
        // Keep the long-lived CAPI credential out of URLs, proxy access logs,
        // provider error messages and exception strings.
        headers: {
    ...
    if (!response.ok) {
      ...
      return { ok: false, status: response.status, retryable: response.status === 429 || response.status >= 500 || result?.error?.is_transient === true || [1, 2, 4, 17, 32, 613].includes(result?.error?.code) };
    }
    ...
    return { ok: Number(result.events_received) > 0, eventsReceived: result.events_received, retryable: true };
  } catch (err) {
...
function buildCompleteRegistration(input) {
  return buildEvent({
    ...input,
...
function sendCompleteRegistration(input) { return send(buildCompleteRegistration(input)); }
function sendPurchase(input) { return send(buildPurchase(input)); }
function buildCheckout(input) { return { ...buildPurchase(input), event_name: 'InitiateCheckout' }; }
function buildActivation(input) {
  return buildEvent({ ...input, eventName: 'Activation', customData: {
    analysis_mode: clean(input.feature, 30), humanize_mode: clean(input.mode, 40),
    chars: Math.max(0, Number(input.chars) || 0), activation_scope: clean(input.scope, 40)
  } });
}
let durableOutbox;
function outbox() {
  if (!durableOutbox) durableOutbox = require('./metaOutbox').createOutbox({ db: require('../config').db, send, logger });
  return durableOutbox;
}
async function queueCompleteRegistration(input) {
  if (!enabled()) return { ok: false, skipped: 'META_CAPI_ACCESS_TOKEN_MISSING' };
  return outbox().enqueue(buildCompleteRegistration(input), input.externalId);
}
