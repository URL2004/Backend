...
const CATALOG = {
  ...
  'payment.product_retired_rejected': { sev: SEV3, domain: 'payment', action: '종료 상품(2,900·8,700) 또는 문의 전용 상품(116,000) 결제 시도. 프런트 배포 뒤 1시간 안에 0으로 수렴해야 한다. 지속되면 캐시·구형 탭 안내 확인.' },
  'payment.first_purchase_lookup_failed': { sev: SEV3, domain: 'payment', action: '첫 구매 판정용 주문 조회 실패. 결제는 정상 처리됐고 플래그만 null — 반복되면 Firestore 상태 확인.' },
  ...
  // ── 결제 실패의 세 갈래(2026-09-04 알림 증폭 사고 대응) ────────────────
  // 예전에는 "실패"가 한 덩어리라 잔액부족 이탈과 전 사용자 결제 불능이 같은 🚨로 나갔다.
  // 분류는 lib/paymentFailureTaxonomy.js가 하고, 등급은 여기서 정한다.
  'payment.customer_declined': { sev: SEV3, domain: 'payment', action: '카드사·계좌 사유 거절(잔액부족·한도초과·세션 만료). 돈은 나가지 않았고 크레딧도 없다. 개별 대응 불필요 — 짧은 시간에 서로 다른 uid가 몰리면 결제사 장애를 의심한다.' },
  'payment.confirm_recovered_by_lookup': { sev: SEV3, domain: 'payment', action: '승인 응답만 놓치고 주문 조회로 실제 상태를 확인했다. 결과 처리는 계속된다 — 잦으면 Toss 응답 지연을 확인.' },
  'payment.provider_rejected_request': { sev: SEV1, domain: 'payment', action: '결제사가 우리 요청 자체를 거부(시크릿 키·가맹점 설정·요청 형식). 전 사용자 결제 불능이다. TOSS_SECRET_KEY와 가맹점 상태를 즉시 확인.' },
  'payment.reconciliation_abandoned_closed': { sev: SEV3, domain: 'payment', action: '거절로 끝난 주문(ABORTED·EXPIRED)을 정산 대상에서 종결했다. 조치 불필요.' },
  'payment.reconciliation_worker_failed': { sev: SEV2, domain: 'payment', action: '정산 워커가 한 건을 처리하지 못했다. manualReview=true면 재시도 소진 또는 불일치 — orderId로 Toss 상태를 대조한다.' },
  'payment.checkout_preclaim_failed': { sev: SEV2, domain: 'payment', action: '결제 시작 선점 실패 — 사용자가 결제창에 들어가지 못한다. Firestore 상태 확인.' },
  'payment.checkout_preclaim_conflict': { sev: SEV3, domain: 'payment', action: '같은 주문번호를 두 번 선점하려 함(중복 클릭 등). 사용자는 재시도로 진행된다 — 급증하면 프런트 주문번호 생성 버그를 의심.' },
  'payment.checkout_context_failed': { sev: SEV2, domain: 'payment', action: '충전 화면 혜택 조회 실패 — 가격·보너스가 안 보인다. Firestore 상태 확인.' },
  'payment.upgrade_offer_resolve_failed': { sev: SEV2, domain: 'payment', action: '업그레이드 대상 주문 조회 실패 — 사용자가 업그레이드 결제를 못 한다.' },
  'payment.intent_blocked_account_unavailable': { sev: SEV3, domain: 'payment', action: '탈퇴 처리 중이거나 계정이 없어 새 결제를 막았다. 정상 차단 — 반복되면 탈퇴 작업 적체를 확인.' },
  'payment.reconciliation_cron_secret_missing': { sev: SEV1, domain: 'cron', action: 'CRON_SECRET 미설정 — 승인·미지급 정산이 멈춘다. env 즉시 확인.' },
  'payment.reconciliation_cron_auth_rejected': { sev: SEV3, domain: 'cron', action: '정산 cron 인증 거부 기록. 실제 중단은 heartbeat로 판정한다.' },
  'payment.reconciliation_cron_failed': { sev: SEV2, domain: 'cron', action: '정산 cron 실행 실패 — 미지급 주문이 남는다. 다음 주기에 복구되지 않으면 수동 실행.' },
  'payment.reconciliation_process_due_rejected': { sev: SEV2, domain: 'cron', action: '구독 크론에 붙은 결제 정산 단계가 거부됐다. cron 시크릿·권한 확인.' },
  'payment.reconciliation_process_due_failed': { sev: SEV2, domain: 'cron', action: '구독 크론의 결제 정산 단계 실패 — 미지급 주문이 남을 수 있다.' },
  ...
  'refund.eligibility_review_not_recorded': { sev: SEV3, domain: 'refund', action: '구형 관리자 화면이 환불 검토 근거를 보내지 않았다. 미확인 상태로 보존됨 — 관리자 UI 배포 뒤 0으로 수렴해야 한다.' },
  'refund.subscription_cancel_status_unknown': { sev: SEV1, domain: 'refund', action: '구독 환불 응답이 유실됐다. Toss 콘솔에서 실제 취소 여부를 대조하고 수동 종결.' },
  ...
  'subscription.start_recovery_claim_failed': { sev: SEV2, domain: 'subscription', action: '구독 시작 복구 클레임 실패. 다음 주기에 재시도되지만 반복되면 수동 확인.' },
  'subscription.start_recovery_apply_failed': { sev: SEV1, domain: 'subscription', action: '첫 결제는 됐는데 복구 적용이 실패했다. 해당 uid에 플랜·크레딧을 수동 지급.' },
  'subscription.start_claim_rejected': { sev: SEV3, domain: 'subscription', action: '동시 요청으로 구독 시작 클레임이 거부됨. 사용자는 재시도로 진행된다.' },
  'subscription.start_operation_failed': { sev: SEV2, domain: 'subscription', action: '구독 시작 처리 실패 — 사용자가 구독을 시작하지 못한다.' },
  'subscription.billing_key_issue_unknown': { sev: SEV1, domain: 'subscription', action: '빌링키 발급 결과 불명 — 카드가 등록됐는데 우리가 모를 수 있다. Toss 콘솔에서 빌링키 존재를 확인하고 수동 종결.' },
  'subscription.billing_issue_unknown_mark_failed': { sev: SEV2, domain: 'subscription', action: '빌링키 불명 상태를 기록하는 것마저 실패. 해당 주문을 수동 검토 목록에 올린다.' },
  'subscription.billing_key_issue_malformed': { sev: SEV1, domain: 'subscription', action: '빌링키 응답 형식이 예상과 다름 — 구독 청구 불가. 결제사 응답 스펙 변경 여부 확인.' },
  'subscription.first_charge_unknown': { sev: SEV1, domain: 'subscription', action: '첫 청구 결과 불명 — 결제됐는데 구독 미적용일 수 있다. orderId로 Toss 상태 대조.' },
  'subscription.first_charge_malformed': { sev: SEV1, domain: 'subscription', action: '첫 청구 응답 형식 이상 — 지급 보류됨. 수동 확인 필요.' },
  'subscription.renewal_claim_rejected': { sev: SEV3, domain: 'subscription', action: '동시 실행으로 갱신 클레임 거부. 다음 주기에 처리된다.' },
  'subscription.renewal_operation_failed': { sev: SEV2, domain: 'subscription', action: '갱신 처리 실패 — 다음 주기 재시도. 반복되면 구독이 조용히 멈춘다.' },
  'subscription.missing_key_quarantine_failed': { sev: SEV2, domain: 'subscription', action: '빌링키 분실 구독의 격리 기록 실패. 청구가 계속 시도될 수 있다.' },
  'subscription.charge_result_unknown': { sev: SEV1, domain: 'subscription', action: '갱신 청구 결과 불명 — 이중청구 위험. 같은 주문키로만 재시도되지만 Toss에서 승인 여부를 직접 확인한다.' },
  'subscription.charge_malformed': { sev: SEV1, domain: 'subscription', action: '갱신 청구 응답 형식 이상 — 사이클 미적용 가능. 수동 확인.' },
  'subscription.expiry_claim_failed': { sev: SEV3, domain: 'subscription', action: '만료 처리 클레임 실패. 다음 주기에 재시도된다.' },
  'subscription.expiry_finalize_failed': { sev: SEV2, domain: 'subscription', action: '구독 만료 종결 실패 — 해지됐는데 혜택이 남을 수 있다.' },
  'subscription.billing_key_delete_failed': { sev: SEV2, domain: 'subscription', action: '만료 구독의 빌링키 삭제 실패 — 카드 정보가 남는다. 다음 주기 재시도 후에도 남으면 수동 삭제.' },
  'subscription.cron_legacy_auth_observed': { sev: SEV3, domain: 'cron', action: '구형 인증 방식으로 들어온 구독 cron 관측. 스케줄러 전환이 끝나면 0이어야 한다.' },
  'billing.secret_read_failed': { sev: SEV2, domain: 'billing', action: '빌링 시크릿 읽기 실패 — 구독 청구·해지가 막힐 수 있다. billingSecrets 확인.' },
  ...
  'toss.webhook_manual_review_required': { sev: SEV1, domain: 'webhook', action: '웹훅 재처리가 한도를 넘겨 수동 검토로 격리됐다. webhookInbox에서 해당 건을 직접 종결한다.' },
  'toss.webhook_retry_failed': { sev: SEV2, domain: 'webhook', action: '웹훅 재처리 실패. 누적되면 정산 불일치 — inbox 적체를 확인한다.' },
  'toss.webhook_subscription_stale_generation_ignored': { sev: SEV3, domain: 'webhook', action: '지난 세대 구독 웹훅을 무시했다. 정상 동작 — 급증하면 세대 관리 버그를 의심.' },
  'toss.webhook_billing_deleted_stale_generation_ignored': { sev: SEV3, domain: 'webhook', action: '지난 세대 카드삭제 웹훅을 무시했다. 정상 동작.' },
  ...
  'detect_report.scoring_unavailable': { sev: SEV3, domain: 'engine', action: '감지 모델 최종 실패로 무차감 503이 반환됨. 반복·급증 시 모델 공급자와 런타임 상태 확인.' },
  ...
  'account.initialize_quota_exceeded': { sev: SEV3, domain: 'account', action: '가입 quota 차단 추세와 공유 NAT 오탐 여부를 함께 확인.' },
  ...
  'qna.created': { sev: SEV3, domain: 'support', action: '문의 저장 완료. 문의 ID로 디스코드 등록 알림과 대조하세요.' },
  'qna.deleted': { sev: SEV3, domain: 'support', action: '문의 삭제 완료. 기존 디스코드 등록 알림은 남아 있을 수 있어요.' },
  'qna.discord_notify_failed': { sev: SEV3, domain: 'support', action: '문의는 저장됐지만 디스코드 알림 전송에 실패했어요. 문의 목록과 웹훅 설정을 확인하세요.' },
