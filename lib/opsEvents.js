...
const CATALOG = {
  ...
  'credit_cancellation.completed_with_unrecovered_credits': { sev: SEV1, domain: 'refund', action: '환불은 됐는데 크레딧 회수 실패 = 실손해. 해당 uid 잔액 수동 조정.' },
  'credit_cancellation.inbox_retry_failed': { sev: SEV2, domain: 'refund', action: '취소 재정산 재시도 실패. 누적되면 정산 불일치.' },
  ...
  'subscription.cron_secret_rejected': { sev: SEV1, domain: 'cron', action: '★과거 실사고 경로: cron 시크릿 불일치로 갱신이 조용히 멈춘다. 스케줄러 헤더와 env 대조.' },
  ...
  'ops.cron_auth_rejected': { sev: SEV2, domain: 'cron', action: '운영 cron 인증 실패 — 워치독이 조용히 멈춘다. 스케줄러 헤더 확인.' },
  ...
  'revenue.cron_auth_rejected': { sev: SEV2, domain: 'cron', action: '매출 cron 인증 실패 — 리포트가 조용히 끊긴다. 스케줄러 헤더 확인.' },
...
function domainFromEvent(event) {
  ...
  const map = {
    ...
    credit_cancellation: 'refund', billing: 'billing', revenue: 'ops', admin: 'ops'
  };
  return map[head] || 'ops';
}
