...
const CATALOG = {
  ...
  'subscription.charge_status_unknown': { sev: SEV1, domain: 'subscription', action: '청구 응답이 유실돼 승인 여부 불명. 같은 orderId를 새로 청구하지 말고 자동 재정산 상태 확인.' },
  'subscription.intent_prepare_failed': { sev: SEV2, domain: 'subscription', action: 'Toss 호출 전 구독 결제 의도 저장 실패. 결제는 시도되지 않았으므로 Firestore 상태 확인.' },
  'subscription.provider_approval_persist_failed': { sev: SEV1, domain: 'subscription', action: 'Toss 승인 뒤 내부 승인 표식 저장 실패. paymentIntents orderId의 자동 재정산 여부 즉시 확인.' },
  'subscription.provider_approval_mismatch': { sev: SEV1, domain: 'subscription', action: 'Toss 승인 주문번호·금액이 내부 의도와 불일치. 자동 적용 금지 상태이므로 Toss 콘솔과 대조.' },
  'subscription.apply_deferred_for_reconciliation': { sev: SEV1, domain: 'subscription', action: 'Toss 결제는 승인됐지만 구독 혜택 적용이 지연됨. 다음 process-due 자동 복구와 orderId 확인.' },
  'subscription.intent_apply_mark_failed': { sev: SEV2, domain: 'subscription', action: '구독 주문 적용 뒤 intent 완료 표식 실패. 자동 재정산은 주문 멱등 확인 후 표식만 복구함.' },
  'subscription.reconciliation_recovered': { sev: SEV3, domain: 'subscription', action: '승인 후 미적용 구독을 자동 복구함. 개별 대응 불필요, 발생 추세 관찰.' },
  'subscription.reconciliation_deferred': { sev: SEV2, domain: 'subscription', action: '구독 자동 재정산이 다음 주기로 미뤄짐. 반복 orderId는 provider 상태와 Firestore 오류 확인.' },
  'subscription.reconciliation_manual_review': { sev: SEV1, domain: 'subscription', action: '구독 자동 재정산 한도 초과 또는 영구 충돌. Toss 결제와 subscriptionOrders를 대조해 수동 종결.' },
  'subscription.reconciliation_scan_failed': { sev: SEV1, domain: 'subscription', action: '승인 후 미적용 구독 재정산 스캔 전체 실패. 다음 cron 전 즉시 Firestore/인덱스 확인.' },
