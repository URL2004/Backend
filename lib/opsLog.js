...
// opsEvents를 require하면 순환이 되므로 등급 문자열만 로컬 상수로 둔다.
const SEV3_LEVEL = 'SEV3';
...
function extract(record) {
  ...
  const accountQuotaEvent = record.event === 'account.initialize_quota_exceeded';
  const scoringUnavailableEvent = record.event === 'detect_report.scoring_unavailable';
  return {
    ...
    uid: accountQuotaEvent || scoringUnavailableEvent ? undefined : record.uid ? cut(record.uid, 128) : undefined,
    actorUid: accountQuotaEvent || scoringUnavailableEvent ? undefined : record.actorUid ? cut(record.actorUid, 128) : undefined,
    orderId: record.orderId ? cut(record.orderId, 128) : undefined,
    jobId: record.jobId ? cut(record.jobId, 128) : undefined,
    questionId: record.questionId ? cut(record.questionId, 160) : undefined,
    ...
    // ── 결제 실패 분류(2026-09-04) ──────────────────────────────────────
    // 관리자 장애 로그에서 "깨울 일인지"를 Render 로그를 다시 뒤지지 않고 판단하게 한다.
    // outcome=customer_declined + moneyAtRisk=false면 그 줄에서 판단이 끝난다.
    outcome: record.outcome ? cut(record.outcome, 40) : undefined,
    failureCategory: record.failureCategory ? cut(record.failureCategory, 40) : undefined,
    actionRequired: record.actionRequired ? cut(record.actionRequired, 20) : undefined,
    moneyAtRisk: typeof record.moneyAtRisk === 'boolean' ? record.moneyAtRisk : undefined,
    creditsGranted: pickNumber(record.creditsGranted),
    providerStatus: record.providerStatus ? cut(record.providerStatus, 40) : undefined,
    providerMessage: record.providerMessage ? cut(record.providerMessage, 300) : undefined,
    providerMethod: record.providerMethod ? cut(record.providerMethod, 40) : undefined,
    // 재시도 상관관계 — "한 사람의 반복"인지 "여러 사람의 장애"인지 한 줄에서 구분한다.
    retryUidFailures5m: pickNumber(record.retryUidFailures5m),
    retryUidDistinctOrders5m: pickNumber(record.retryUidDistinctOrders5m),
    retryOrderAttempt: pickNumber(record.retryOrderAttempt),
    repeatedDecline: typeof record.repeatedDecline === 'boolean' ? record.repeatedDecline : undefined,
    surgeSubjects: pickNumber(record.surgeSubjects),
    occurrences: pickNumber(record.occurrences),
    confirmLatencyMs: pickNumber(record.confirmLatencyMs),
    lookupLatencyMs: pickNumber(record.lookupLatencyMs),
    elapsedMs: pickNumber(record.elapsedMs),
    ...
    quotaAction: accountQuotaEvent && record.action ? cut(record.action, 80) : undefined,
    quotaScope: accountQuotaEvent && record.scope ? cut(record.scope, 20) : undefined,
    quotaCount: accountQuotaEvent ? pickNumber(record.count) : undefined,
    quotaLimit: accountQuotaEvent ? pickNumber(record.limit) : undefined,
    grantCredits: accountQuotaEvent ? pickNumber(record.grantCredits) : undefined,
    upstreamCode: scoringUnavailableEvent && record.upstreamCode ? cut(record.upstreamCode, 80) : undefined,
    latencyMs: scoringUnavailableEvent ? pickNumber(record.latencyMs) : undefined,
    lengthBucket: scoringUnavailableEvent && record.lengthBucket ? cut(record.lengthBucket, 24) : undefined,
...
// 급증 판정의 주체 키. 같은 사람·같은 주문의 되풀이를 한 명으로 센다.
//
// 왜(2026-09-04): 한 사용자가 잔액부족으로 3번 재시도했을 뿐인데 payment 도메인 5건 임계를
// 넘겨 SEV1 급증(@here)이 나갔다. 건수만 세면 "한 사람의 반복"과 "여러 사람이 겪는 장애"가
// 구분되지 않는다. 사람을 특정할 수 없는 사건(인프라 등)은 종전처럼 건별로 센다.
let anonymousSeq = 0;
function subjectKey(entry) {
  if (entry.uid) return `uid:${entry.uid}`;
  if (entry.orderId) return `order:${entry.orderId}`;
  return `evt:${++anonymousSeq}`;
}

// 급증 감지: 창 안에서 서로 다른 주체 수가 임계를 넘으면 { domain, subjects, count, threshold } 반환.
function bumpRate(domain, nowMs, key) {
  const threshold = RATE_THRESHOLDS[domain];
  if (!threshold) return null;
  const arr = (rateHits.get(domain) || []).filter(hit => nowMs - hit.t < RATE_WINDOW_MS);
  arr.push({ t: nowMs, key });
  rateHits.set(domain, arr);
  const subjects = new Set(arr.map(hit => hit.key));
  if (subjects.size < threshold) return null;
  ...
  return {
    ...
    count: arr.length,
    threshold,
    windowMinutes: Math.round(RATE_WINDOW_MS / 60000)
  };
}
...
function makeGroupKey(entry) {
  ...
  const authSuffix = entry.authReason
    ...
    : '';
  // UID별 문서를 만들지는 않되 quota 종류와 시간/일일 범위는 섞지 않는다.
  const quotaSuffix = entry.event === 'account.initialize_quota_exceeded'
    ? `|${entry.quotaAction || ''}|${entry.quotaScope || ''}`
    : '';
  const scoringSuffix = entry.event === 'detect_report.scoring_unavailable'
    ? `|${entry.upstreamCode || ''}`
    : '';
  // 결제 실패는 사유별로 대응이 다르다. 잔액부족과 한도초과를 한 문서로 합치면
  // 관리자 화면에서 "무엇이 몰리는지"가 사라진다.
  const failureSuffix = entry.failureCategory ? `|${entry.failureCategory}` : '';
  return `${entry.event}|${entry.severity}${authSuffix}${quotaSuffix}${scoringSuffix}${failureSuffix}`;
}

function record(record_, classification) {
  const nowMs = Date.now();
  const base = stripUndefined(extract(record_));
  ...
  pruneGroups(nowMs);
  // 급증은 병합 여부와 무관하게 센다. 단 두 가지는 제외한다.
  //  · 급증 사건 자신(무한 승격 방지)
  //  · SEV3 — 카드사 거절 같은 "정상 이탈"이다. 이것으로 SEV1을 만들면 알림이 스스로를 깨운다.
  const surge = (entry.event === 'ops.rate_threshold_exceeded' || severity === SEV3_LEVEL)
    ? null
    : bumpRate(domain, nowMs, subjectKey(entry));
...
function groupCount(event, severity, details) {
  const g = openGroups.get(makeGroupKey({
    ...
    quotaAction: details && (details.quotaAction || details.action),
    quotaScope: details && (details.quotaScope || details.scope),
    upstreamCode: details && details.upstreamCode,
    failureCategory: details && details.failureCategory
  }));
  return g ? g.count : 1;
}
