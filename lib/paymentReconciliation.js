'use strict';

const crypto = require('crypto');

const ORDER_ID_PATTERN = /^[A-Za-z0-9_-]{6,64}$/;
const PAYMENT_KEY_MAX_LENGTH = 300;

function sha256(value) {
  return crypto.createHash('sha256').update(String(value)).digest('hex');
}

function paymentKeyHash(paymentKey) {
  return sha256(paymentKey);
}

function confirmIdempotencyKey(orderId, paymentKey) {
  return `credit-confirm-${sha256(`${orderId}\u0000${paymentKey}`)}`;
}

function refundOperationId(orderId, priorRefundedAmount, refundAmount, refundCredits, operationEpoch = null) {
  const parts = [
    orderId,
    Number(priorRefundedAmount) || 0,
    Number(refundAmount) || 0,
    Number(refundCredits) || 0
  ];
  // 같은 환불 신청의 재시도는 동일 키를 써야 하지만, 결제사가 확정 거절한 뒤
  // 크레딧을 복원하고 새로 접수한 신청은 과거 실패 응답을 재사용하면 안 된다.
  // 신규 request lifecycle만 sequence를 epoch로 넣고 레거시/직접 환불 해시는 보존한다.
  if (operationEpoch !== null && operationEpoch !== undefined && String(operationEpoch) !== '') {
    parts.push(String(operationEpoch));
  }
  return `credit-refund-${sha256(parts.join('\u0000'))}`;
}

function refundIdempotencyKey(operationId) {
  return `credit-cancel-${sha256(operationId)}`;
}

function providerCanceledAmount(payment) {
  const value = payment && typeof payment === 'object' ? payment : {};
  const total = Number(value.totalAmount);
  const balance = Number(value.balanceAmount);
  if (Number.isFinite(total) && total >= 0 && Number.isFinite(balance) && balance >= 0 && balance <= total) {
    return Math.floor(total - balance);
  }
  const cancels = Array.isArray(value.cancels) ? value.cancels : [];
  return cancels.reduce((sum, cancel) => {
    const amount = Number(cancel && cancel.cancelAmount);
    return sum + (Number.isFinite(amount) && amount > 0 ? Math.floor(amount) : 0);
  }, 0);
}

function refundedCreditsForCanceledAmount({ orderAmount, purchasedCredits, canceledAmount }) {
  const amount = Math.max(0, Math.floor(Number(orderAmount) || 0));
  const credits = Math.max(0, Math.floor(Number(purchasedCredits) || 0));
  const canceled = Math.max(0, Math.floor(Number(canceledAmount) || 0));
  if (!amount || !credits || !canceled) return 0;
  if (canceled >= amount) return credits;
  return Math.min(credits, Math.floor((credits * canceled) / amount));
}

function creditCancellationGrant(order) {
  const value = order && typeof order === 'object' ? order : {};
  const legacyTotalCredits = Math.max(0, Math.floor(Number(value.safeCredits ?? value.credits) || 0));
  const declaredTotalCredits = Math.max(0, Math.floor(Number(
    value.totalGrantedCredits ?? value.safeCredits ?? value.credits
  ) || 0));
  const explicitPaidCredits = Math.max(0, Math.floor(Number(value.paidCredits) || 0));
  const usesBaseCreditPolicy = String(value.creditGrantPolicyVersion || '') === 'credit-grant-base-v1'
    && explicitPaidCredits > 0
    && declaredTotalCredits >= explicitPaidCredits;
  // Orders without the explicit v1 marker must keep the historical safeCredits/credits
  // source order exactly, even if an unrelated or partially migrated field exists.
  const totalCredits = usesBaseCreditPolicy ? declaredTotalCredits : legacyTotalCredits;
  const paidCredits = usesBaseCreditPolicy ? explicitPaidCredits : totalCredits;
  return {
    usesBaseCreditPolicy,
    totalCredits,
    paidCredits,
    bonusCredits: Math.max(0, totalCredits - paidCredits),
    refundCreditBasis: usesBaseCreditPolicy ? 'paid_credits_first' : 'legacy_total_grant'
  };
}

function cancellationCreditTarget({ orderAmount, canceledAmount, order }) {
  const grant = creditCancellationGrant(order);
  if (!grant.usesBaseCreditPolicy) {
    return {
      ...grant,
      targetPaidCredits: 0,
      bonusRevocationTargetCredits: 0,
      targetCredits: refundedCreditsForCanceledAmount({
        orderAmount,
        purchasedCredits: grant.totalCredits,
        canceledAmount
      })
    };
  }

  // New grants are refunded against paid credits, and any cancellation revokes the
  // order's entire event bonus. The current wallet balance must never reduce this
  // target: it only caps the debit below. Otherwise a customer could spend the bonus,
  // cancel externally, and make the spent bonus disappear from unrecoveredCredits.
  const targetPaidCredits = refundedCreditsForCanceledAmount({
    orderAmount,
    purchasedCredits: grant.paidCredits,
    canceledAmount
  });
  return {
    ...grant,
    targetPaidCredits,
    bonusRevocationTargetCredits: grant.bonusCredits,
    targetCredits: Math.min(
      grant.totalCredits,
      targetPaidCredits + grant.bonusCredits
    )
  };
}

function cancellationLedgerId(orderId, canceledAmount) {
  return `provider_refund_${sha256(`${orderId}\u0000${Number(canceledAmount) || 0}`)}`;
}

function buildCreditCancellationPlan({ payment, order, currentCredits, knownRefundLedgerCredits = 0, userExists = true }) {
  const providerStatus = String(payment && payment.status || '');
  if (!['CANCELED', 'PARTIAL_CANCELED'].includes(providerStatus)) {
    return { applicable: false, reason: 'provider_status_not_canceled' };
  }
  const canceledAmount = providerCanceledAmount(payment);
  const orderAmount = Math.max(0, Math.floor(Number(order && order.amount) || 0));
  const creditTarget = cancellationCreditTarget({ orderAmount, canceledAmount, order });
  const purchasedCredits = creditTarget.totalCredits;
  if (!canceledAmount || !orderAmount || !purchasedCredits) {
    return { applicable: false, reason: 'invalid_cancellation_amounts' };
  }

  let targetCredits = creditTarget.targetCredits;
  const existingCredits = Math.max(0, Math.floor(Number(order && order.refundedCredits) || 0));
  const knownLedgerCredits = Math.max(0, Math.floor(Number(knownRefundLedgerCredits) || 0));
  const processing = order && order.refundProcessing && typeof order.refundProcessing === 'object'
    ? order.refundProcessing
    : null;
  const processingTarget = Math.max(0, Math.floor(Number(processing && processing.targetRefundedCredits) || 0));
  const processingTargetAmount = Math.max(0, Math.floor(Number(processing && processing.targetRefundedAmount) || 0));
  const processingMatchesProvider = Boolean(
    processing
    && processingTargetAmount > 0
    && processingTargetAmount <= canceledAmount
    && processingTarget <= purchasedCredits
  );
  if (processing && processingTargetAmount > canceledAmount) {
    return { applicable: false, reason: 'pending_refund_not_provider_confirmed' };
  }
  if (processingMatchesProvider) targetCredits = Math.max(targetCredits, processingTarget);
  const processingPriorCredits = processingMatchesProvider
    ? Math.max(0, Math.floor(Number(processing.priorRefundedCredits) || 0))
    : 0;
  const processingReserved = processingMatchesProvider
    ? Math.min(
      Math.max(0, processingTarget - processingPriorCredits),
      Math.max(0, Math.floor(Number(processing.creditsToDeduct) || 0))
    )
    : Math.max(0, Math.min(targetCredits, existingCredits) - knownLedgerCredits);
  // A request-time reservation has already removed these credits from both the
  // wallet and its lot.  The provider webhook can arrive after cancellation but
  // before the approve route persists refundedCredits/refundedAmount, so the
  // immutable processing operation must participate in accountedCredits.
  const processingAccountedCredits = processingMatchesProvider
    ? Math.min(targetCredits, processingPriorCredits + processingReserved)
    : 0;
  const accountedCredits = Math.max(existingCredits, knownLedgerCredits, processingAccountedCredits);
  // Provider cancellation totals are cumulative. Never lower the recovery target
  // below credits already reserved or reconciled when a later webhook arrives after
  // the user has spent more of the remaining balance.
  targetCredits = Math.max(targetCredits, accountedCredits);
  const unaccountedCredits = Math.max(0, targetCredits - accountedCredits);
  const availableCredits = userExists ? Math.max(0, Math.floor(Number(currentCredits) || 0)) : 0;
  const balanceDebit = Math.min(availableCredits, unaccountedCredits);
  const appliedCredits = accountedCredits + balanceDebit;

  return {
    applicable: true,
    providerStatus,
    orderStatus: canceledAmount >= orderAmount ? 'refunded' : 'partially_refunded',
    canceledAmount,
    refundCreditBasis: creditTarget.refundCreditBasis,
    paidCredits: creditTarget.paidCredits,
    bonusCredits: creditTarget.bonusCredits,
    targetPaidCredits: creditTarget.targetPaidCredits,
    bonusRevocationTargetCredits: creditTarget.bonusRevocationTargetCredits,
    targetCredits,
    existingCredits,
    knownLedgerCredits,
    accountedCredits,
    processingReserved,
    balanceDebit,
    ledgerCredits: Math.max(0, Math.min(processingReserved, targetCredits - knownLedgerCredits)) + balanceDebit,
    appliedCredits,
    unrecoveredCredits: Math.max(0, targetCredits - appliedCredits),
    operationId: processing && typeof processing.operationId === 'string' ? processing.operationId : null,
    clearProcessing: processingMatchesProvider
  };
}

function validateConfirmInput({ paymentKey, orderId }) {
  if (typeof paymentKey !== 'string' || !paymentKey.trim() || paymentKey.length > PAYMENT_KEY_MAX_LENGTH) {
    return { ok: false, error: '결제 식별자가 올바르지 않습니다.' };
  }
  if (typeof orderId !== 'string' || !ORDER_ID_PATTERN.test(orderId)) {
    return { ok: false, error: '주문 식별자가 올바르지 않습니다.' };
  }
  return { ok: true };
}

function approvedPaymentValidation(payment, expected) {
  const result = payment && typeof payment === 'object' ? payment : {};
  const reasons = [];
  if (result.orderId !== expected.orderId) reasons.push('order_id_mismatch');
  if (result.paymentKey !== expected.paymentKey) reasons.push('payment_key_mismatch');
  if (Number(result.totalAmount) !== Number(expected.amount)) reasons.push('amount_mismatch');
  if (result.status !== 'DONE') reasons.push('status_not_done');
  return {
    ok: reasons.length === 0,
    reasons,
    status: typeof result.status === 'string' ? result.status : null,
    totalAmount: Number.isFinite(Number(result.totalAmount)) ? Number(result.totalAmount) : null
  };
}

function webhookPaymentValidation(payment, reported) {
  const result = payment && typeof payment === 'object' ? payment : {};
  const payload = reported && typeof reported === 'object' ? reported : {};
  const reasons = [];
  if (!payload.paymentKey || result.paymentKey !== payload.paymentKey) reasons.push('payment_key_mismatch');
  if (payload.orderId && result.orderId !== payload.orderId) reasons.push('order_id_mismatch');
  return {
    ok: reasons.length === 0,
    reasons,
    orderId: typeof result.orderId === 'string' ? result.orderId : null,
    status: typeof result.status === 'string' ? result.status : null
  };
}

function providerResultSummary(result) {
  const value = result && typeof result === 'object' ? result : {};
  // 실패한 주문 조회는 top-level code/message가 비어 있고 failure 안에 진짜 사유가 들어온다.
  // 2026-09-04 로그에 code:null·message:null만 남아 "왜 ABORTED인지"를 Toss 콘솔에서
  // 따로 봐야 했던 원인이 이것이다.
  const failure = value.failure && typeof value.failure === 'object' ? value.failure : {};
  const easyPay = value.easyPay && typeof value.easyPay === 'object' ? value.easyPay : {};
  return {
    code: typeof value.code === 'string' ? value.code : null,
    message: typeof value.message === 'string' ? value.message.slice(0, 300) : null,
    status: typeof value.status === 'string' ? value.status : null,
    orderId: typeof value.orderId === 'string' ? value.orderId : null,
    totalAmount: Number.isFinite(Number(value.totalAmount)) ? Number(value.totalAmount) : null,
    balanceAmount: Number.isFinite(Number(value.balanceAmount)) ? Number(value.balanceAmount) : null,
    approvedAt: typeof value.approvedAt === 'string' ? value.approvedAt : null,
    requestedAt: typeof value.requestedAt === 'string' ? value.requestedAt : null,
    failureCode: typeof failure.code === 'string' ? failure.code : null,
    failureMessage: typeof failure.message === 'string' ? failure.message.slice(0, 300) : null,
    method: typeof value.method === 'string' ? value.method : null,
    easyPayProvider: typeof easyPay.provider === 'string' ? easyPay.provider : null
  };
}

function creditLedgerDelta(row) {
  const amount = Number(row && row.amount);
  const used = Number(row && row.used);
  const safeAmount = Number.isFinite(amount) ? amount : 0;
  const safeUsed = Number.isFinite(used) ? used : 0;
  // Historical admin debits store the same debit as amount=-N and used=N.
  // The balance changed by -N, not -2N.
  // New lot-aware admin debits use the common usage-ledger shape amount=0,
  // used=N. Prefer an explicit non-zero amount for legacy/positive adjustments,
  // otherwise derive the debit from used exactly once.
  if (row && row.type === 'admin_adjust') {
    return safeAmount !== 0 ? safeAmount : -Math.max(0, safeUsed);
  }
  return safeAmount - safeUsed;
}

module.exports = {
  ORDER_ID_PATTERN,
  approvedPaymentValidation,
  buildCreditCancellationPlan,
  cancellationCreditTarget,
  cancellationLedgerId,
  confirmIdempotencyKey,
  creditLedgerDelta,
  paymentKeyHash,
  providerCanceledAmount,
  providerResultSummary,
  refundedCreditsForCanceledAmount,
  refundIdempotencyKey,
  refundOperationId,
  validateConfirmInput,
  webhookPaymentValidation
};
