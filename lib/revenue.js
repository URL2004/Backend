...
const { confirmedRefundAmount } = require('./refundAccounting');
...
function refundedOf(o, amt) {
  return Math.min(amt, confirmedRefundAmount(o));
}
  ...
  for (const o of rows || []) {
    ...
    switch (o.status) {
      ...
      case 'refund_rejected': { // 환불 거절 후 결제가 유지된 상태 → 매출에 포함
        ...
        const b = bucket(amt); b.paidAmount += amt - refunded; b.paidCount += 1;
        if (refunded > 0) {
