...
const PRINCIPAL_THRESHOLDS = Object.freeze({
  soft: Object.freeze({ hourly: 4, daily: 20 }),
...
function balanceBucket(remaining, grantCredits) {
  ...
  if (remaining < grantCredits) return 'eleven_to_twenty_four';
  return 'full';
}
...
function completedDetectHumanize24(events, grantCredits, remainingCredits) {
  ...
  for (const event of events) {
    ...
    if (event.eventType === 'spend' && step === detectStep && event.creditAmount === 8) detectObserved = true;
    if (detectObserved && event.eventType === 'spend'
      && step === basicHumanizeStep && event.creditAmount === 16) {
      orderedPair = true;
    }
  }
  return orderedPair
    && (netByStep.get(detectStep) || 0) >= 8
    && (netByStep.get(basicHumanizeStep) || 0) >= 16
    && remainingCredits <= Math.max(0, grantCredits - 24);
}
...
function emptyCohort({ windowHours, sinceMs, nowMs }) {
  return {
    ...
    detectHumanize24: { accounts: 0, rate: 0 },
    balanceBuckets: { zero: 0, one: 0, two_to_five: 0, six_to_ten: 0, eleven_to_twenty_four: 0, full: 0 },
...
function aggregateCohort(events, { windowHours, nowMs }) {
  ...
  for (const account of byAccount.values()) {
    ...
    if (completedDetectHumanize24(account.events, grantCredits, remaining)) completed24 += 1;
  }
  ...
  return {
    ...
    detectHumanize24: { accounts: completed24, rate: roundedRate(completed24, accounts) },
    balanceBuckets,
    spend: {
