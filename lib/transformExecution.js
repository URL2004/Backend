...
function createExecutionCoordinator({ db, caps, clock = Date.now }) {
  ...
  async function acquire(job, feature) {
    ...
    return change(async (stored, tx) => {
      ...
      if (tx) {
        ...
        const deletion = await tx.get(db.collection('accountDeletionJobs').doc(job.uid));
  ...
  async function renew(lease) {
    return change(stored => {
      ...
      return { slots, value: owned };
    });
  }
  async function release(lease) {
    return change(stored => {
      const slots = live(stored, clock());
      if (slots[lease.key]?.token === lease.token) delete slots[lease.key];
      return { slots, value: true };
    });
  }
