...
function serializeAdminLedgerTaskHistory(id, history) {
  ...
  return {
    ...
    scoreAdjustment: require('../lib/detectScorePresentation').sanitizeAdjustment(row.scoreAdjustment, row.probability),
    probSource: ['llm', 'cached_llm'].includes(row.probSource) ? row.probSource : '',
    detectConfidence: ['low', 'medium', 'high'].includes(row.detectConfidence) ? row.detectConfidence : '',
    detectModel: typeof row.detectModel === 'string' ? row.detectModel.slice(0, 80) : '',
    detectorVersion: typeof row.detectorVersion === 'string' ? row.detectorVersion.slice(0, 80) : '',
    detectEscalated: typeof row.detectEscalated === 'boolean' ? row.detectEscalated : null,
...
router.post('/admin/user-history-item', async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, item: {
      ...
      scoreAdjustment: require('../lib/detectScorePresentation').sanitizeAdjustment(h.scoreAdjustment, h.probability),
