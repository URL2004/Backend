...
test('감지 이력은 공개 점수 provenance만 allowlist로 저장한다', async () => {
  ...
  await history.saveAnalyzeHistory({
    ...
    result: {
      ...
      modelProbability: 81,
      statisticalSupport: {
        version: 'statistical-assist-v1', applied: true, originalScore: 32, score: 62,
        margin: 0.5, features: 400, profile: 'general', rawText: 'statistics-private-sentinel'
      },
      detectDiagnostics: {
  ...
  assert.equal(stored.value.detectStatisticalSupport.originalScore, 32);
  assert.equal(JSON.stringify(stored.value.detectStatisticalSupport).includes('statistics-private-sentinel'), false);
