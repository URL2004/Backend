...
test('글 종류가 불확실하면 두 축을 아예 비활성(off)으로 두고 이유만 적는다 — 막대·등급 없음', () => {
  ...
  assert.equal(unknown.axes.anchor.status, 'off');
  assert.equal(unknown.axes.stance.status, 'off');
  assert.match(unknown.axes.stance.reason, /가리지 못해/u);
  assert.equal(unknown.axes.anchor.target, null);
  // 프로필은 알지만 신뢰도가 낮아도 같은 취급
  const shaky = resolveRadarAxisPolicy({ profile: 'resume_application', confidence: 0.3, sentenceTotal: 15 });
  assert.equal(shaky.axes.stance.status, 'off');
...
test('정책표는 모든 프로필에 anchor·stance를 갖고 target은 0~1 범위다', () => {
  for (const [profile, spec] of Object.entries(AXIS_POLICY_BY_PROFILE)) {
    for (const axis of ['anchor', 'stance']) {
      ...
      assert.ok(entry && ['on', 'off'].includes(entry.status), `${profile}.${axis} status — soft(참고) 단계는 없다`);
      if (entry.status !== 'off') assert.ok(entry.target > 0 && entry.target <= 1, `${profile}.${axis} target`);
      if (entry.status === 'off') assert.ok(entry.reason && /(해요|않아요|않았어요)$/u.test(entry.reason), `${profile}.${axis} off reason 해요체`);
    }
  }
});
...
test('보고서 뷰는 글 종류를 받아 measuredEvidence.axisPolicy로 내려보내고, 없으면 unknown 정책을 쓴다', () => {
  ...
  assert.equal(without.measuredEvidence.axisPolicy.axes.anchor.status, 'off');
});
